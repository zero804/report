...
public class SimpleForm extends DwContentPanel {
	...
	public class SField {
		...
		public void setFieldLayoutConfig(
	...
	protected List<String> fieldKeyList = new ArrayList<String>();
	protected Map<String, SField> fieldLookup = new HashMap<String, SField>();
	...
	/**
	 * Called once after the form has been rendered for the first time and loads dependencies.
	 * 
	 * <p>
	...
	public DwTextButton addSubmitButton(String txt) {
		...
		/* add ok button */
		submitButton = new DwTextButton(txt);
	 ...
	 * 
	 * <p>If no group was previously started.
	...
	public String addField(Class<?> type, ValueProvider vp, String label, Object value, SimpleFormFieldConfiguration... configs){
		return addField(type, vp.getPath(), FormFieldLayoutConfiguration.from(label, fieldLayoutConfig), value, vp, configs);
	}

	public String addField(Class<?> type, String name, String label, Object value, SimpleFormFieldConfiguration... configs){
	...
	public static FormFieldProviderHook getResponsibleHooker(Class<?> type,
			SimpleFormFieldConfiguration... configs) {
		List<FormFieldProviderHook> providers = hookService.getHookers(FormFieldProviderHook.class);
		for(FormFieldProviderHook provider : providers){
			if(provider.consumes(type, configs)){
				return provider;
			}
		}
	...
	protected void loadConditions() {
		for(ConditionRegistrar registrar : conditionRegistrars)
			registrar.registerCondition();
	}
	...
	private void addToContainer(Container lc, Widget widget, double width, double height, MarginData margin, FormFieldLayoutConfiguration config) {
		if(lc instanceof VerticalLayoutContainer)
			((VerticalLayoutContainer)lc).add(widget, new VerticalLayoutData(width,height, null == margin ? widget.getLayoutData() instanceof HasMargins ? ((HasMargins)widget.getLayoutData()).getMargins() : null != config ? config.getMarginData().getMargins() : new Margins(0) : margin.getMargins()));
		else if(lc instanceof HorizontalLayoutContainer)
			((HorizontalLayoutContainer)lc).add(widget, new HorizontalLayoutData(width,height, null == margin ? widget.getLayoutData() instanceof HasMargins ? ((HasMargins)widget.getLayoutData()).getMargins() : null != config ? config.getMarginData().getMargins() : new Margins(0,10,0,00) : margin.getMargins()));
		else if(lc instanceof HBoxLayoutContainer){
			BoxLayoutData data = new BoxLayoutData(null == margin ? widget.getLayoutData() instanceof HasMargins ? ((HasMargins)widget.getLayoutData()).getMargins() : null != config ? config.getMarginData().getMargins() : new Margins(0,10,0,0) : margin.getMargins());
			data.setFlex(width);
	...
	public List<SField> getAllSFields(){
		List<SField> fields = new ArrayList<SimpleForm.SField>();

		for(String key : fieldLookup.keySet())
			fields.add(getSField(key));
	...
	private void formSubmitted() {
		if(isValidateOnSubmit() && ! mainForm.isValid()){
			new DwAlertMessageBox(FormsMessages.INSTANCE.validationFailedTitle(), FormsMessages.INSTANCE.validationFailedMessage()).show();
			return;
		}
	...
	protected String addField(SimpleFormFieldJson fieldConfig) {
		...
		String name = addField(responsibleHook, fieldConfig.getId(), FormFieldLayoutConfiguration.from(fieldConfig.getLabel(), fieldLayoutConfig), null, null);
	...
	public static FormFieldProviderHook getResponsibleHooker(String type, SimpleFormFieldJson config) {
		List<FormFieldProviderHook> providers = hookService.getHookers(FormFieldProviderHook.class);
		for(FormFieldProviderHook provider : providers){
			if(provider.consumes(type, config)){
				return provider;
			}
		}
