...
public class StaticListProvider extends BaseListProvider {
	...
	interface ListTemplates extends XTemplates {
	    ...
	    "<div class=\"{style.staticlistDescription}\">{entry.description:nullsafe}</div>" +
	    "</div>")
	...
	public boolean doConsumes(Class<?> type, SimpleFormFieldConfiguration... configs) {
		return 
			...
			null != configs && 
			(configs[0] instanceof SFFCStaticList);
	}
...
//		comboBox.setAllowTextSelection(((SFFCStaticList)configs[0]).allowTextSelection());
		...
		if(isString){
			...
			((SimpleComboBox<KeyValueContainer>)comboBox).addSelectionHandler(new SelectionHandler<StaticListProvider.KeyValueContainer>() {
				@Override
	...
	protected Widget createDropDown() {
		ListStore<FancyModel> store = new ListStore<StaticListProvider.FancyModel>(new BasicObjectModelKeyProvider<FancyModel>());
		...
		if(configs[0] instanceof SFFCFancyStaticList){
			...
			ListView<FancyModel, FancyModel> view = new ListView<FancyModel, FancyModel>(
					store, new IdentityValueProvider<FancyModel>(), GWT.<ListViewAppearance<FancyModel>>create(RepserListViewFancyAppearance.class));
					*/
			ListView<FancyModel, FancyModel> view = new ListView<FancyModel, FancyModel>(store, new IdentityValueProvider<FancyModel>());
			
			view.setCell(new AbstractCell<FancyModel>() {
				@Override
				public void render(com.google.gwt.cell.client.Cell.Context context,
						FancyModel value, SafeHtmlBuilder sb) {
					sb.append(template.render(
						value,
						SafeStylesUtils.fromTrustedString("height: " + SafeHtmlUtils.htmlEscape(value.getHeight()) + "px;"),
						staticListResources.listStyles()
					));
				}
			});
			
			combo = new ComboBox<FancyModel>(new ComboBoxCell<FancyModel>(store, new StringLabelProvider<FancyModel>(), view));
	...
	private void addFieldBindingsRadio(Object model, ValueProvider vp,
			Widget field) {
	...
	private void addFieldBindingsDropdown(Object model, ValueProvider vp,
			Widget field) {
