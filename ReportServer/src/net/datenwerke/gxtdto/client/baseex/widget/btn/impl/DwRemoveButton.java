	...
	private void init() {
			...
			public void onSelect(SelectEvent event) {
				...
				if(! isConfirmSingleItem())
					...
					SafeHtmlBuilder sb = new SafeHtmlBuilder()
							.appendEscaped(getRemoveConfirmMessage())
							.appendHtmlConstant("<br/><br/>")
							.appendEscaped(getRemovePromptDescription());
					
					final PromptMessageBox pmb = new DwPromptMessageBox(SafeHtmlUtils.fromString(getRemoveConfirmTitle()), sb.toSafeHtml());
					pmb.setWidth(400);
					...
					pmb.addDialogHideHandler(new DialogHideHandler() {
						...
						public void onDialogHide(DialogHideEvent event) {
							if (event.getHideButton() == PredefinedButton.OK && "REMOVE".equals(pmb.getTextField().getValue())){ 
								onRemove();
							}
						}
					});
		...
		removeAllButton.addSelectionHandler(new SelectionHandler<Item>() {
			...
			public void onSelection(SelectionEvent<Item> event) {
				...
				if(isProtectAll()){
					SafeHtmlBuilder sb = new SafeHtmlBuilder()
							.appendEscaped(getRemoveAllConfirmMessage())
							.appendHtmlConstant("<br/><br/>")
							.appendEscaped(getRemovePromptDescription());
					
					final PromptMessageBox pmb = new DwPromptMessageBox(SafeHtmlUtils.fromString(getRemoveAllConfirmTitle()), sb.toSafeHtml());
					pmb.setWidth(400);
					pmb.getButton(PredefinedButton.OK).disable();
					pmb.getTextField().addValueChangeHandler(new ValueChangeHandler<String>() {
						@Override
					...
					ConfirmMessageBox cmb = new DwConfirmMessageBox(getRemoveAllConfirmTitle(), getRemoveAllConfirmMessage());
					cmb.addDialogHideHandler(new DialogHideHandler() {
						@Override
						public void onDialogHide(DialogHideEvent event) {
							if (event.getHideButton() == PredefinedButton.YES){ 
