...
public class DwTabPanel extends TabPanel {
	...
	protected void onItemContextMenu(final Widget item, int x, int y) {
	    ...
	    if (closeContextMenu == null) {
	        ...
	        closeContextMenu.add(new DwMenuItem(BaseMessages.INSTANCE.closeOtherTabsText(), new SelectionHandler<MenuItem>() {
	          @Override
	          public void onSelection(SelectionEvent<MenuItem> event) {
	        ...
	        closeContextMenu.add(new DwMenuItem(BaseMessages.INSTANCE.closeAllsTabText(), new SelectionHandler<MenuItem>() {
		          @Override
		          public void onSelection(SelectionEvent<MenuItem> event) {
		        	  final List<Widget> widgets = new ArrayList<Widget>();
	...
	protected void askClose(final List<Widget> widgets) {
		...
		if(! unsavedChanges)
			...
			ConfirmMessageBox cmb = new DwConfirmMessageBox(BaseMessages.INSTANCE.warning(), BaseMessages.INSTANCE.unsavedChanges());
