...
public class BasicObjectImporter {
	...
	@Inject
	public BasicObjectImporter(
		HookHandlerService hookHandler,
		ReflectionService reflectionServices,
		@Assisted ImportSupervisor importSupervisor,
		@Assisted ImportableElement exportedItem
		){
	...
	protected void importProperties(Object obj, Collection<ItemProperty> properties) throws SecurityException, NoSuchFieldException, IllegalArgumentException, IllegalAccessException, InstantiationException {
		...
		if(processedProperties.size() != properties.size()){
			for(ItemProperty property : properties)
				if(! processedProperties.contains(property) && allowImportField(obj, getField(obj, property)))
					throw new IllegalStateException("I could apparently not import all properties: " + obj.getClass() + ", property: " + property.getName());
		}
	}
	...
	protected Object importPropertyCollection(ItemPropertyCollection property) throws InstantiationException, IllegalAccessException, SecurityException, NoSuchFieldException  {
