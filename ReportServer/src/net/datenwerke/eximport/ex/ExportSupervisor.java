...
public class ExportSupervisor {
	...
	private void configureExporters() {
		...
		while(configChanged){
			configChanged = false;
			for(Exporter exporter : exporters){
				Collection<ExportItemConfig<?>> configs = CollectionUtils.subtract(configMap.get(exporter), processedConfigs.keySet());
				if(configs.isEmpty())
					continue;
	...
	public String getReferenceId(Object value) {
		...
		if(! hasEncounteredId(id))
			throw new IllegalStateException("The encountered id has not been registered: " + id + ", " + value.getClass().getName());
	...
	public void exportEnclosedObject(Object enclosed) throws XMLStreamException {
		...
		if(! enclosedConfigs.contains(config))
			throw new IllegalArgumentException("The enclosed object " + enclosed.getClass() + " with Id" + config.getId() + " has not been registered.");
	...
	public void createPropertyElement(String propertyName, Class<?> propertyType, Object value) throws XMLStreamException {
	...
	public void beginPropertyCollectionElement(String propertyName, Class<?> propertyType, Class<? extends Collection> collectionType) throws XMLStreamException {
	...
	public void addReferenceTo(Object value, boolean optional) throws XMLStreamException {
		...
		if(null == exporter)
			throw new IllegalStateException("Found a reference without finding an exporter to take responsibility: " + value.getClass() + ", id:" + id);
