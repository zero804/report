...
public class OlapUtilServiceImpl implements OlapUtilService {
	...
	public OlapUtilServiceImpl(HookHandlerService hookHandlerService,
			JdbcService jdbcService, Provider <ParameterSetFactory> parameterSetFactory,
	...
	@Override
	public List<String> getCubes(MondrianDatasource datasource, SaikuReport report) throws ClassNotFoundException, IOException, SQLException {
	...
	public OlapConnection getOlapConnection(MondrianDatasource mondrianDatasource, SaikuReport report, boolean resetParameters) throws IOException, ClassNotFoundException, SQLException{
		...
		if(XMLA_DRIVER.equals(driver)){
			...
			if(!props.containsKey(XMLA_USERNAME_KEY) && null != mondrianDatasource.getUsername() && ! "".contentEquals(mondrianDatasource.getUsername().trim())){
				props.setProperty(XMLA_USERNAME_KEY, mondrianDatasource.getUsername());
			}
			if(!props.containsKey(XMLA_PASSWORD_KEY) && null != mondrianDatasource.getPassword() && ! "".contentEquals(mondrianDatasource.getPassword().trim())) {
			...
			if (null != mondrianDatasource.getMondrianSchema() && ! "".contentEquals(mondrianDatasource.getMondrianSchema().trim())) {
				/* init parameter set */
				...
				/* use fields if no property defined */
				if(!props.containsKey(MONDRIAN_USERNAME_KEY) && null != mondrianDatasource.getUsername() && ! "".contentEquals(mondrianDatasource.getUsername().trim())){
					props.setProperty(MONDRIAN_USERNAME_KEY, mondrianDatasource.getUsername());
				}
				if(!props.containsKey(MONDRIAN_PASSWORD_KEY) && null != mondrianDatasource.getPassword() && ! "".contentEquals(mondrianDatasource.getPassword().trim())) {
		...
		if(!props.containsKey(ISaikuConnection.URL_KEY) && null != mondrianDatasource.getUrl() && ! "".contentEquals(mondrianDatasource.getUrl().trim())) {
