...
public class ImportMainPanel extends DwContentPanel {
	...
	public ImportMainPanel(
		...
		ImporterDao importerDao,
		NavigationPanelHelper navPanelHelper,
		Provider<PropertiesDialog> propertiesDialogProvider,
	...
	private void showBeginToolbar() {
		...
		/* start import button */
		DwTextButton beginImportBtn = toolbarService.createSmallButtonLeft(ExImportMessages.INSTANCE.beginImportButtonLabel(), BaseIcon.IMPORT);
	...
	private void initToolbar(){
		...
		/* main options */
		DwTextButton mainOptionsBtn = toolbarService.createSmallButtonLeft(ExImportMessages.INSTANCE.mainOptionsButtonLabel(), BaseIcon.COG);
		...
		/* abort import button */
		DwTextButton resetImportBtn = toolbarService.createSmallButtonLeft(ExImportMessages.INSTANCE.importResetButtonLabel(), BaseIcon.ROTATE_RIGHT);
		...
		DwTextButton abortImportBtn = toolbarService.createSmallButtonLeft(ExImportMessages.INSTANCE.importAbortButtonLabel(), BaseIcon.DELETE);
		...
		DwTextButton submitImportBtn = toolbarService.createSmallButtonLeft(ExImportMessages.INSTANCE.importSubmitButtonLabel(), BaseIcon.IMPORT);
	...
	protected void configureAndImport() {
		...
		importerDao.performImport(configMap, postProcessorMap, new NotamCallback<Void>(ExImportMessages.INSTANCE.importSuccess()){
			@Override
	...
	protected void clearConfig() {
		ConfirmMessageBox cmb = new DwConfirmMessageBox(ExImportMessages.INSTANCE.clearConfigTitle(), ExImportMessages.INSTANCE.clearConfigMsg());
	...
	private void initImporterNavPanel(Collection<String> ids){
		...
		TreeStore<NavigationModelData<ImporterConfiguratorHook>> store = new TreeStore<NavigationModelData<ImporterConfiguratorHook>>(new BasicObjectModelKeyProvider<NavigationModelData<ImporterConfiguratorHook>>());
		
		/* add modules */
		for(ImporterConfiguratorHook importer : importers)
			store.add(new NavigationModelData<ImporterConfiguratorHook>(importer.getImporterName(), importer.getImporterIcon(), importer));
		
		/* build tree */
		final Tree<NavigationModelData<ImporterConfiguratorHook>, String> tree = navPanelHelper.createNavigationTreePanel(store, new NavItemSelectionCallback<ImporterConfiguratorHook>() {
			@Override
	...
	private void loadPostProcessors() {
		...
		for(ImporterPostProcessorConfiguratorHook postProcessor : hookHandler.getHookers(ImporterPostProcessorConfiguratorHook.class))
	...
	protected void displayBeginImport() {
		...
		final String fileUploadKey = form.addField(FileUpload.class, RsExImportUiModule.IMPORT_DATA_FILE_NAME, ExImportMessages.INSTANCE.uploadImportFileDataLabel(), new SFFCFileUpload() {
			@Override
			public UploadProperties getProperties() {
				UploadProperties uploadProperties = new UploadProperties("importdata", RsExImportUiModule.UPLOAD_HANDLER_ID);
		...
		final String dataKey = form.addField(String.class, ExImportMessages.INSTANCE.uploadImportDataLabel(), new SFFCTextArea(){
