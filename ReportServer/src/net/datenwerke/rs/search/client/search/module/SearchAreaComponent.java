...
public class SearchAreaComponent extends DwContentPanel {
	...
	private void initListPanel() {
		...
		loader = renewLoader();
		loader.addLoadHandler(new LoadResultListStoreBinding<PagingLoadConfig, SearchResultEntryDto, PagingLoadResult<SearchResultEntryDto>>(store));
		loader.addLoadHandler(new LoadHandler<PagingLoadConfig, PagingLoadResult<SearchResultEntryDto>>(){
			@Override
			public void onLoad(
					LoadEvent<PagingLoadConfig, PagingLoadResult<SearchResultEntryDto>> event) {
				listPanel.unmask();
			}
		});
		...
		ColumnConfig<SearchResultEntryDto,SearchResultEntryDto> iconColumn = new ColumnConfig<SearchResultEntryDto,SearchResultEntryDto>(new IdentityValueProvider<SearchResultEntryDto>(), 30);
		iconColumn.setCell(new AbstractCell<SearchResultEntryDto>() {
			@Override
			public void render(com.google.gwt.cell.client.Cell.Context context,
					SearchResultEntryDto model, SafeHtmlBuilder sb) {
		...
		ColumnConfig<SearchResultEntryDto,SearchResultEntryDto> idColumn = new ColumnConfig<SearchResultEntryDto,SearchResultEntryDto>(new IdentityValueProvider<SearchResultEntryDto>("__id"), 90, BaseMessages.INSTANCE.id());
		...
		ColumnConfig<SearchResultEntryDto,SearchResultEntryDto> entryColumn = new ColumnConfig<SearchResultEntryDto,SearchResultEntryDto>(new IdentityValueProvider<SearchResultEntryDto>("__entry"), 200, SearchMessages.INSTANCE.entryColumnLabel());
		...
		entryColumn.setCell(new AbstractCell<SearchResultEntryDto>() {
			...
			public void render(com.google.gwt.cell.client.Cell.Context context,
				...
				String title = null == value.getTitle() ? BaseMessages.INSTANCE.unknown() : Format.ellipse(value.getTitle(), 100);
				String description = null == value.getDescription() || "".equals(value.getDescription())? BaseMessages.INSTANCE.unknown() : Format.ellipse(value.getDescription(), 1000);
				sb.appendHtmlConstant("<div class=\"rs-list-name-desc\"><div class=\"title\">" + title + "</div><div class=\"description\">" + description + "</div></div>");
			}
		});
		entryColumn.setMenuDisabled(true);
		entryColumn.setSortable(false);
		
		ColumnConfig<SearchResultEntryDto, SearchResultEntryDto> ccLastModified = new ColumnConfig<SearchResultEntryDto,SearchResultEntryDto>(new IdentityValueProvider<SearchResultEntryDto>("__last"), 150, BaseMessages.INSTANCE.lastModified()); 
		ccLastModified.setMenuDisabled(true);
		ccLastModified.setCell(new AbstractCell<SearchResultEntryDto>() {
			@Override
			public void render(com.google.gwt.cell.client.Cell.Context context, SearchResultEntryDto model, SafeHtmlBuilder sb) {
		...
		/* create grid */
		final Grid<SearchResultEntryDto> grid = new Grid<SearchResultEntryDto>(store, new ColumnModel<SearchResultEntryDto>(columns));
		...
		grid.getSelectionModel().addSelectionChangedHandler(new SelectionChangedHandler<SearchResultEntryDto>() {
			...
			public void onSelectionChanged(
					SelectionChangedEvent<SearchResultEntryDto> event) {
	...
	protected PagingLoader<PagingLoadConfig,PagingLoadResult<SearchResultEntryDto>> renewLoader() {
		RpcProxy<PagingLoadConfig, PagingLoadResult<SearchResultEntryDto>> proxy = new RpcProxy<PagingLoadConfig, PagingLoadResult<SearchResultEntryDto>>() {  
			...
			public void load(
	    	  ...
	    	  searchDao.find(searchService.enhanceQuery(searchField.getValue()), filter, new RsAsyncCallback<SearchResultListDto>(){
		    		@Override
	    ...
	    final PagingLoader<PagingLoadConfig,PagingLoadResult<SearchResultEntryDto>> loader = new PagingLoader<PagingLoadConfig,PagingLoadResult<SearchResultEntryDto>>(proxy);  
