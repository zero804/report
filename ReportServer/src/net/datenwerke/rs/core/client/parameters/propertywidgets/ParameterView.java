...
public class ParameterView extends ReportExecutorMainPanelView implements DeselectionAwareView {
	...
	public VerticalLayoutContainer getParameterContainer(){
		...
		for(final ParameterDefinitionDto definition : parameterDefinitions){
			...
			/* get edit form */
			Widget editWidget = configurator.getEditComponentForInstance(instance, definition, relevantInstances, true, labelWidth, getExecuteReportToken());
						...
						if(aDefinition.getDependsOn().contains(definition)){
							...
							Scheduler.get().scheduleDeferred(new Command() {
								public void execute() {
									...
									Widget editWidget = configurator.getEditComponentForInstance(instance, aDefinition, relevantInstances, false, fLabelWidth, getExecuteReportToken());
