...
public class ReportExecutorUIServiceImpl implements ReportExecutorUIService {
	...
	public ReportExecutorUIServiceImpl(
		...
		Provider<ReportExecuteAreaModule> executeModuleProvider,
		DwMainViewportUiService viewportService,
		final ReportExecutorDao reportExecutorDao,
		Provider<VariantStorerConfigImpl> variantStorerConfigProvider,
		WaitOnEventUIService waitOnEventService,
		Provider<ImageReportPreviewView> imagePreviewProvider,
		Provider<JsViewerReportPreviewView> jsPreviewProvider,
		Provider<NativeReportPreviewView> nativePreviewProvider,
		TsDiskDao tsDiskDao
		){
	...
	@Override
	public void executeReport(final ReportDto report, final ExecutorEventHandler eventHandler, final ExecuteReportConfiguration config, final ReportViewConfiguration... viewConfigs) {
		String addParams = null != config  ? config.getUrlParameters() : null;
		History.newItem(ReportExecutorUIModule.REPORT_EXECUTOR_HISTORY_TOKEN + "/uuid:" + report.getUuid() + (null != addParams ? "&" + addParams : ""), false);
	...
	@Override
	public void executeReportDirectly(ReportDto report, final ExecutorEventHandler eventHandler, final ExecuteReportConfiguration config, final ReportViewConfiguration... viewConfigs) {
		viewportService.unmask();
		...
		CompositeExecutorEventHandler theEventHandler = new CompositeExecutorEventHandler(
	...
	public String createExecuteReportToken(ReportDto report) {
		...
		String token = report.getId() + "-" + loginService.getCurrentUser().getId() + "-" + new Date().getTime() + "-" + r.nextInt();
		return token;
	}
	...
	public void createNewVariant(ReportDto report, TeamSpaceDto teamSpace, TsDiskFolderDto folder, String executeToken, String name, String description,
		...
		if(null != teamSpace)
			tsDiskDao.createAndImportVariant(teamSpace, folder, report, executeToken, name, description, new RsAsyncCallback<TsDiskReportReferenceDto>(){
				@Override
