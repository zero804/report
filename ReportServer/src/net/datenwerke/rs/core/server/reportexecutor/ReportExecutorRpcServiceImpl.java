 ...
 */
@Singleton
public class ReportExecutorRpcServiceImpl extends SecuredRemoteServiceServlet
		implements ReportExecutorRpcService {
	 ...
	 * @see net.datenwerke.rs.core.client.reportexecutor.rpc.ReportExecutorRpcService#editVariant(net.datenwerke.rs.web.client.reportmanager.dto.reports.ReportDTO, java.lang.String, java.lang.String, boolean)
	 */
	@Override
	@Transactional(rollbackOn={Exception.class})
	public ReportDto editVariant(ReportDto reportVariantDto, String executeToken, String title, String description) throws ServerCallFailedException{
		...
		/* test rights */
		if(! securityService.checkRights(referenceVariant.getParent(), Execute.class))
			throw new ViolatedSecurityException();
	...
	public void deleteVariant(ReportDto reportVariantDto) throws ServerCallFailedException{
		...
		/* test rights */
		if(! securityService.checkRights(referenceVariant.getParent(), Delete.class, Execute.class))
			throw new ViolatedSecurityException();		
	...
	@Override
	@Transactional(rollbackOn={Exception.class})
	public DwModel executeAs(String format, String executeToken, @Named("report") ReportDto report, DwModel config) throws ServerCallFailedException {
		for(ReportExecutorExecuteAsHooker hooker : hookHandler.getHookers(ReportExecutorExecuteAsHooker.class)){
			if(hooker.consumes(report, format)){
				...
				ReportExecutorExecuteAsHooker.ExecuteConfig execConfig = hooker.getConfig(report, currentUser, format, config);
	...
	@Override
	@Transactional(rollbackOn={Exception.class})
	public SuccessIndicatorBaseModel storePNGInSession(String executorToken, @Named("report") ReportDto reportDto) throws ServerCallFailedException {
		if (null == reportDto.getId()) 
			throw new RuntimeException(new ReportExecutorException(ReportManagerMessages.INSTANCE.exceptionReportCouldNotBeLoaded()));
		...
		try {
			CompiledPngReport res = (CompiledPngReport) reportExecutor.execute(referenceReport, ReportExecutorService.OUTPUT_FORMAT_PNG, new RECFirstPage());
	...
	public ReportDto loadReportForExecutionFrom(HistoryLocation location) throws ServerCallFailedException {
		...
		/* adjust variant */
		for(ConfigureReportViaHistoryLocationHook adjuster : hookHandler.getHookers(ConfigureReportViaHistoryLocationHook.class))
	...
	private Object execute(@Named("report")ReportDto reportDto, String outputFormat, Collection<ReportExecutionConfig> configs) throws ServerCallFailedException {
		...
		try {
			...
			Object res = reportExecutor.execute(referenceReport, outputFormat, configs.toArray(new ReportExecutionConfig[]{}));
			return res;
		} catch (ReportExecutorException e){
