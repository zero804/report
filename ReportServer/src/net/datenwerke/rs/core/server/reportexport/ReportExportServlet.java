...
public class ReportExportServlet extends SecuredHttpServlet{
	...
	protected void displayErrorMessage(Exception e, HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		try{
			...
			try{
				...
				if(e instanceof ReportNotExistsException)
					msg = errorHelperProvider.get().getHtmlErrorPage(ReportManagerMessages.INSTANCE.exceptionReportDoesNotExist(), e);
				else
					msg = errorHelperProvider.get().getHtmlErrorPage(ReportManagerMessages.INSTANCE.exceptionWhileReportExecution(), e);
				writer.append(msg);
			} finally {
				writer.flush();
				writer.close();
	...
	protected void exportBackLinkReport(HttpServletRequest req,
			HttpServletResponse resp) throws IOException, ReportExecutorException {
		...
		if(null != backLinkReport)
			exportReport(backLinkReport.getReportId(), backLinkReport.getAdjustedReport(), backLinkReport.getOutputFormat(), null, backLinkReport, req, resp, null);
	}



	protected void exportReportByIdViaRequest(HttpServletRequest req, HttpServletResponse resp) throws IOException, ReportExecutorException {
		long reportId = getReportId(req);
		exportReportById(reportId, req, resp);
	}
	
	private void exportReportById(long reportId, HttpServletRequest req, HttpServletResponse resp) throws IOException, ReportExecutorException {
	...
	private ReportExecutionConfig[] getConfigsFromRequest(Report report, HttpServletRequest req) {
		List<ReportExecutionConfig> configs = new ArrayList<ReportExecutionConfig>();
		
		for(ReportExecutionConfigFromPropertyMapHook configProvider : hookHandlerProvider.get().getHookers(ReportExecutionConfigFromPropertyMapHook.class))
			configs.addAll(configProvider.parse(report, req, req.getParameterMap()));
		
		return configs.toArray(new ReportExecutionConfig[]{});
	}
	...
	private void exportReportViaSession(HttpServletRequest req,	HttpServletResponse resp) throws IOException, ReportExecutorException {
		/* retrieve token */
		...
		if(entry.isCached()){
			...
			if(null == tempFile)
				throw new IllegalStateException("Report could not be loaded from cache, because the previous execution failed. See below for original cause. ", entry.getError()); //$NON-NLS-1$
	...
	private void exportReport(long reportId, Report adjustedReport, String outputFormat, String suggestedFileName, BackLinkReport backLinkReport, HttpServletRequest req, HttpServletResponse resp, ReportSessionCacheEntry entry, ReportExecutionConfig... reportExecutorConfigs) throws IOException, ReportExecutorException {
		try {
			doExportReport(reportId, adjustedReport, outputFormat, suggestedFileName, backLinkReport, req, resp, entry, reportExecutorConfigs);
			if(null != entry)
				entry.setError(null);
		} catch(Exception e){
			if(null != entry) 
	...
	private void doExportReport(long reportId, Report adjustedReport, String outputFormat, String suggestedFileName, BackLinkReport backLinkReport, HttpServletRequest req, HttpServletResponse resp, ReportSessionCacheEntry entry, ReportExecutionConfig... reportExecutorConfigs) throws IOException, ReportExecutorException {
		...
		if(null != req.getParameter(PARAMETER_BACKLINK_ID) || null != backLinkReport){
			String backlinkid = null != backLinkReport ? backLinkReport.getPredecessor() : req.getParameter(PARAMETER_BACKLINK_ID);
...
//		resp.setDateHeader("Expires", 1);	
		...
		if(supportsStreaming(referenceReport, backLinkSet, outputFormat, reportExecutorConfigs, req)){
			/* execte report dry */
			CompiledReport dryReport = exeucteReportDry(referenceReport, backLinkSet, outputFormat, reportExecutorConfigs, req);
			
			String baseFilename = (null == suggestedFileName || "".equals(suggestedFileName.trim()))?makeExportFilename(referenceReport):suggestedFileName;
			...
			for(ReportExportAdjustHttpHeaderHook hooker: hookHandlerProvider.get().getHookers(ReportExportAdjustHttpHeaderHook.class))
	...
	private void storeInCache(String rexToken, String baseFilename, String mimetype, String extension, Object executedReport, HttpServletRequest req, ReportSessionCacheEntry entry) throws FileNotFoundException, IOException{
		if(null == entry)
			return;
	...
	private String getReportExecutorToken(ReportExecutionConfig[] reportExecutorConfigs) {
		if(null == reportExecutorConfigs)
			return null;
		for(ReportExecutionConfig cfg : reportExecutorConfigs)
			if(cfg instanceof RECReportExecutorToken)
				return ((RECReportExecutorToken) cfg).getToken();

		return null;
	}

	protected boolean supportsStreaming(Report report,
			ParameterSet backLinkSet, String outputFormat,
	...
	protected CompiledReport exeucteReportDry(Report report,
			ParameterSet backLinkSet, String outputFormat,
			ReportExecutionConfig[] reportExecutorConfigs, HttpServletRequest req) throws ReportExecutorException {
	...
	private String storeBackLinkReport(HttpServletRequest req, long reportId, Report adjustedReport, String outputFormat) {
		HttpSession session = req.getSession();
...
//		session.setAttribute(SESSION_KEY_DOWNLOAD, req.getParameter("download"));
	...
	private Report getReportFromRequest(HttpServletRequest req) {
		long reportId = getReportId(req);
		
		Report report = reportService.get().getUnmanagedReportById(reportId);
		
		for(ConfigureReportViaHttpRequestHook adjuster : hookHandlerProvider.get().getHookers(ConfigureReportViaHttpRequestHook.class))
			adjuster.adjustReport(report, req);
