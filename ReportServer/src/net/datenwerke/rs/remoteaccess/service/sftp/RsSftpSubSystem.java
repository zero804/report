...
public class RsSftpSubSystem implements Command, Runnable, SessionAware, FileSystemAware {

	protected static final Logger log = LoggerFactory.getLogger("sftp");
    ...
    protected static class DirectoryHandle extends Handle implements Iterator<SshFile> {
        ...
        public boolean hasNext() {
            return null != fileList && fileIndex < fileList.size();
        }
    ...
    protected void doProcess(Buffer buffer) throws IOException {
        ...
        switch (type) {
            ...
            case SSH_FXP_OPEN: {
                ...
                if (version <= 4) {
                    ...
                    // attrs
                    try {
                        SshFile file = resolveFile(path, true);
                        if (file.doesExist()) {
            ...
            case SSH_FXP_MKDIR: {
                ...
                // attrs
                try {
                    SshFile p = resolveFile(path, true);
                    if (p.doesExist()) {
                        if (p.isDirectory()) {
    ...
    private SshFile resolveFile(String path) throws IOException {
    	return resolveFile(path, false);
    }
    
    private SshFile resolveFile(String path, boolean allowNonExisting) throws IOException {
		return ((SftpFileSystemView) this.root).getFile(path, allowNonExisting);
    }
