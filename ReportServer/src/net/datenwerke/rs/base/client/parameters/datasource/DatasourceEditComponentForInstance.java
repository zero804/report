...
public class DatasourceEditComponentForInstance {
	...
	@Inject
	public DatasourceEditComponentForInstance(
			DatasourceParameterDao queryRPCService
		){
	...
	public Widget getEditComponent(DatasourceParameterDefinitionDto definition, DatasourceParameterInstanceDto instance, Collection<ParameterInstanceDto> relevantInstances, int labelWidth, boolean initial) {
		/* what mode */
		if(ModeDto.Single.equals(definition.getMode()) && SingleSelectionModeDto.Dropdown.equals(definition.getSingleSelectionMode()))
			return getEditComponent_single(definition, instance, relevantInstances, initial, labelWidth);
		else if(ModeDto.Single.equals(definition.getMode()) && SingleSelectionModeDto.Popup.equals(definition.getSingleSelectionMode()))
			return getEditComponent_popup(definition, instance, relevantInstances, labelWidth);
		else if(ModeDto.Single.equals(definition.getMode()) && SingleSelectionModeDto.Radio.equals(definition.getSingleSelectionMode()))
			return getEditComponent_cb(definition, instance, relevantInstances, initial, false, labelWidth);
		else if(ModeDto.Single.equals(definition.getMode()) && SingleSelectionModeDto.Listbox.equals(definition.getSingleSelectionMode()))
			return getEditComponent_lb(definition, instance, relevantInstances, initial, false, labelWidth);
		else if(ModeDto.Multi.equals(definition.getMode()) && MultiSelectionModeDto.Popup.equals(definition.getMultiSelectionMode()))
			return getEditComponent_popup(definition, instance, relevantInstances, labelWidth);
		else if(ModeDto.Multi.equals(definition.getMode()) && MultiSelectionModeDto.Checkbox.equals(definition.getMultiSelectionMode()))
			return getEditComponent_cb(definition, instance, relevantInstances, initial, true, labelWidth);
		else if(ModeDto.Multi.equals(definition.getMode()) && MultiSelectionModeDto.Listbox.equals(definition.getMultiSelectionMode()))
			return getEditComponent_lb(definition, instance, relevantInstances, initial, true, labelWidth);

		return null;
	}
	
	protected Widget getEditComponent_cb(
			final DatasourceParameterDefinitionDto definition,
			final DatasourceParameterInstanceDto instance,
			Collection<ParameterInstanceDto> relevantInstances, boolean initial, final boolean multi, int labelWidth) {
		final DwHorizontalFlowLayoutContainer container = new DwHorizontalFlowLayoutContainer();
//		container.setNobreak(false);
		
		ListLoader<ListLoadConfig, ListLoadResult<DatasourceParameterDataDto>> loader = getLoader(definition, relevantInstances);
		...
		return new ParameterFieldWrapperForFrontend(definition, instance, container, labelWidth, new DefaultValueSetter() {
			@Override
	...
	protected Widget getEditComponent_lb(
			final DatasourceParameterDefinitionDto definition,
			final DatasourceParameterInstanceDto instance,
			Collection<ParameterInstanceDto> relevantInstances, boolean initial, final boolean multi, int labelWidth) {
		final ListStore<DatasourceParameterDataDto> store = new ListStore<DatasourceParameterDataDto>(new BasicObjectModelKeyProvider());
		...
		ColumnConfig<DatasourceParameterDataDto, String> column = new ColumnConfig<DatasourceParameterDataDto, String>(DatasourceParameterDataDtoPA.INSTANCE.key(), 200, "");
		...
		final Grid<DatasourceParameterDataDto> grid = new Grid<DatasourceParameterDataDto>(store, new ColumnModel<DatasourceParameterDataDto>(columns));
	...
	private void setDefaultValueSelectionModel(
			CheckBoxSelectionModel<DatasourceParameterDataDto> selection,
	...
	protected void setDefaultValueToggleGroup(Container container,
			DatasourceParameterInstanceDto instance,
		...
		if(multi){
			for(Widget vc : container){
				for(Widget w : ((VerticalLayoutContainer)vc)){
					...
					f.setValue(
						null != definition.getMultiDefaultValueSimpleData() && definition.getMultiDefaultValueSimpleData().contains(f.getData(DATA_KEY))
					);
				}
			}
			ArrayList<DatasourceParameterDataDto> multiValue = new ArrayList<DatasourceParameterDataDto>();
			...
			if(null != definition.getSingleDefaultValueSimpleData())
				instance.setSingleValue(DatasourceParameterDataDtoDec.cloneDataObject(definition.getSingleDefaultValueSimpleData()));
			else
				instance.setSingleValue(null);
		}
	...
	protected void fillContainer(Container container,
			final DatasourceParameterDefinitionDto definition,
			final DatasourceParameterInstanceDto instance,
			...
			if(!multi ){
				...
				box.addValueChangeHandler(new ValueChangeHandler<Boolean>() {
					...
					public void onValueChange(ValueChangeEvent<Boolean> event) {
						...
						List<DatasourceParameterDataDto> multiValue = new ArrayList<DatasourceParameterDataDto>(instance.getMultiValue());
				...
				if(! instance.isStillDefault() && (null != instance.getMultiValue() && instance.getMultiValue().contains(dataObject)))
					box.setValue(true, true);
			}
	...
	protected Widget getEditComponent_single(final DatasourceParameterDefinitionDto definition, final DatasourceParameterInstanceDto instance, Collection<ParameterInstanceDto> relevantInstances, boolean initial, int labelWidth) {
		final ListStore<DatasourceParameterDataDto> store = new ListStore<DatasourceParameterDataDto>(new BasicObjectModelKeyProvider());
		final ListLoader<ListLoadConfig, ListLoadResult<DatasourceParameterDataDto>> loader = getLoader(definition, relevantInstances);
	    loader.addLoadHandler(new LoadResultListStoreBinding<ListLoadConfig, DatasourceParameterDataDto, ListLoadResult<DatasourceParameterDataDto>>(store));
		...
		store.addStoreDataChangeHandler(new StoreDataChangeHandler<DatasourceParameterDataDto>() {
			...
			public void onDataChange(
					StoreDataChangeEvent<DatasourceParameterDataDto> event) {
				if(instance.isStillDefault())
		...
		/* load data */
		if(! initial || (instance.isStillDefault() && (null == definition.getDependsOn() || definition.getDependsOn().isEmpty())))
			loader.load();
		else {
			comboBox.setValue(instance.getSingleValue(), true);
			...
			comboBox.addBeforeQueryHandler(new BeforeQueryHandler<DatasourceParameterDataDto>() {
				...
				public void onBeforeQuery(
						BeforeQueryEvent<DatasourceParameterDataDto> event) {
					if(loaded)
						return;
	...
	protected void setDefaultValueSingle(
			ListStore<DatasourceParameterDataDto> store,
			ComboBox<DatasourceParameterDataDto> comboBox,
			DatasourceParameterInstanceDto instance,
			DatasourceParameterDefinitionDto definition) {
	...
	protected Widget getEditComponent_popup(final DatasourceParameterDefinitionDto definition, final DatasourceParameterInstanceDto instance, final Collection<ParameterInstanceDto> relevantInstances, int labelWidth) {
		/* create store and add selected values*/
		final ListStore<DatasourceParameterDataDto> gridStore = new ListStore<DatasourceParameterDataDto>(new BasicObjectModelKeyProvider());

		/* create grid */
		final ListStore<DatasourceParameterDataDto> allStore = new ListStore<DatasourceParameterDataDto>(new BasicObjectModelKeyProvider());
		final SimpleGridSelectionPopup<DatasourceParameterDataDto> multiValueGrid = new SimpleGridSelectionPopup<DatasourceParameterDataDto>(BaseMessages.INSTANCE.name(), DatasourceParameterDataDtoPA.INSTANCE.key(), gridStore, allStore){
			@Override
			protected void itemsSelected(
					List<DatasourceParameterDataDto> selectedItems) {
				if(ModeDto.Single.equals(definition.getMode())){
					if(null != selectedItems && selectedItems.size() > 0)
		...
		multiValueGrid.setAllItemsStoreLoader(getLoader(definition, relevantInstances));
		...
		/* populate values - neeeds to be done in deferred way to allow parameter view to capture change events (parameter view starts listening only after we returned the widget) */
		Scheduler.get().scheduleDeferred(new ScheduledCommand() {
			@Override
			public void execute() {
				if(instance.isStillDefault()) {
					...
					if(ModeDto.Single.equals(definition.getMode()))
						...
						for(DatasourceParameterDataDto value : new ArrayList<DatasourceParameterDataDto>(instance.getMultiValue()))
							gridStore.add(value);
		...
		return new ParameterFieldWrapperForFrontend(definition, instance, multiValueGrid, labelWidth, new DefaultValueSetter() {
			@Override
			public void setDefaultValue() {
				setDefaultValuePopup(gridStore, instance, definition);
			}
	...
	protected void setDefaultValuePopup(
			ListStore<DatasourceParameterDataDto> gridStore,
			DatasourceParameterInstanceDto instance,
			DatasourceParameterDefinitionDto definition) {
		...
		if(ModeDto.Single.equals(definition.getMode())){
			if(null != definition.getSingleDefaultValueSimpleData()){
				DatasourceParameterDataDto clonedValue = DatasourceParameterDataDtoDec.cloneDataObject(definition.getSingleDefaultValueSimpleData());
	 ...
	 * @return
	 */
	private ListLoader<ListLoadConfig, ListLoadResult<DatasourceParameterDataDto>> getLoader(final DatasourceParameterDefinitionDto definition, final Collection<ParameterInstanceDto> relevantInstances) {
		RpcProxy<ListLoadConfig, ListLoadResult<DatasourceParameterDataDto>> proxy = new RpcProxy<ListLoadConfig, ListLoadResult<DatasourceParameterDataDto>>() {
			...
			public void load(
				...
				datasourceParamDao.loadDatasourceParameterData(definition, relevantInstances, false, callback);
			}
		};
	...
	protected boolean isValueSet(
			DatasourceParameterDefinitionDto definition,
			DatasourceParameterInstanceDto instance) {
		if(ModeDto.Single.equals(definition.getMode()) && null == instance.getSingleValue() && ! SingleSelectionModeDto.Dropdown.equals(definition.getSingleSelectionMode()) )
			return false;
