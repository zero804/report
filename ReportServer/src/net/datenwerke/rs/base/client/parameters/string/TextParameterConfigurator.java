...
public class TextParameterConfigurator extends ParameterConfiguratorImpl<TextParameterDefinitionDto, TextParameterInstanceDto> {
	...
	public Widget getEditComponentForDefinition(TextParameterDefinitionDto definition, ReportDto report) {
		...
		form.beginRow();
		final String widthKey = form.addField(Integer.class, TextParameterDefinitionDtoPA.INSTANCE.width(), BaseMessages.INSTANCE.width()); 
		final String heightKey = form.addField(Integer.class, TextParameterDefinitionDtoPA.INSTANCE.height(), BaseMessages.INSTANCE.height());
		form.endRow();
		
		/* add type */
		final String returnTypeKey = form.addField(
			List.class, TextParameterDefinitionDtoPA.INSTANCE.returnType(), RsMessages.INSTANCE.returnType(), //$NON-NLS-1$ //$NON-NLS-2$
		...
		final String blankKey = form.addField(Boolean.class, TextParameterDefinitionDtoPA.INSTANCE.returnNullWhenEmpty(), new SFFCBoolean() {
			@Override
		...
		final String regexKey = form.addField(String.class, TextParameterDefinitionDtoPA.INSTANCE.validatorRegex(), RsMessages.INSTANCE.validatorRegexLabel());
		...
		SFFCStringValidator validator = new SFFCStringValidator() {
			...
			public Validator<String> getValidator() {
				Validator<String> validator = TextParameterConfigurator.this.getValidator((DatatypeDto)form.getValue(returnTypeKey), (String)form.getValue(regexKey), false).getValidator();
				return validator;
			}
		}; 

		String defaultValueKey = form.addField(String.class, TextParameterDefinitionDtoPA.INSTANCE.defaultValue(), ReportmanagerMessages.INSTANCE.defaultValue(),  
			new SFFCTextArea(){
	...
	@Override
	public Widget doGetEditComponentForInstance(final TextParameterInstanceDto instance, Collection<ParameterInstanceDto> relevantInstances, final TextParameterDefinitionDto definition, boolean initial, int labelWidth, String executeReportToken) {
		SimpleForm form = SimpleForm.getInlineLabelessInstance();
		form.setFocusOnShow(false);
		
		/* prepare validator */
		SFFCStringValidator validator = getValidator(definition.getReturnType(), definition.getValidatorRegex(), definition.isMandatory());
		
		/* display text field */
		form.addField(String.class, TextParameterInstanceDtoPA.INSTANCE.value(), definition.getKey(), validator, new SFFCTextArea() { //$NON-NLS-1$
			public int getWidth() {
	...
	@Override
	protected void doSetDefaultValueInInstance(TextParameterInstanceDto instance, TextParameterDefinitionDto definition) {
