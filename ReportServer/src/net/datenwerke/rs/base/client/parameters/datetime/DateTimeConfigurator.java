...
public class DateTimeConfigurator extends ParameterConfiguratorImpl<DateTimeParameterDefinitionDto, DateTimeParameterInstanceDto> {
	...
	public Widget getEditComponentForDefinition(DateTimeParameterDefinitionDto definition, ReportDto report) {
		...
		final String modeKey = form.addField(
				List.class, DateTimeParameterDefinitionDtoPA.INSTANCE.mode(), RsMessages.INSTANCE.mode(), 
		...
		form.addField(Boolean.class, DateTimeParameterDefinitionDtoPA.INSTANCE.useNowAsDefault(), RsMessages.INSTANCE.useNowAsDefault()); 
		String defaultKey = form.addField(Date.class, DateTimeParameterDefinitionDtoPA.INSTANCE.defaultValue(), RsMessages.INSTANCE.defaultValue(), new SFFCDate() { 
			public Mode getMode() {
		...
		form.addField(String.class, DateTimeParameterDefinitionDtoPA.INSTANCE.formula(), RsMessages.INSTANCE.defaultFormula());
		form.endGroup();
	...
	@Override
	public Widget doGetEditComponentForInstance(final DateTimeParameterInstanceDto instance, Collection<ParameterInstanceDto> relevantInstances, final DateTimeParameterDefinitionDto definition, boolean initial, int labelWidth, String executeReportToken) {
		...
		form.addField(DateFormulaContainer.class, DateTimeParameterInstanceDtoDec.getDateFormulaValueProvider(), definition.getKey(), dateConfig,
				new SFFCAllowBlank() {
	...
	@Override
	protected void doSetDefaultValueInInstance(DateTimeParameterInstanceDto instance, DateTimeParameterDefinitionDto definition) {
		/* note that it is important to set the formula first, as the event is thrown by changing the value */
