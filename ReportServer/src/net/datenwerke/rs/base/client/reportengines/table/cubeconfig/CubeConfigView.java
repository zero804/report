...
public class CubeConfigView extends ReportExecutorMainPanelView {
	...
	private void createToolbar() {
		...
		DwTextButton selectColumnsButton = toolbarService.createSmallButtonLeft(
				TableMessages.INSTANCE.selectMeasures(), BaseIcon.TABLE_ADD
		);
		...
		selectColumnsButton.addSelectHandler(new SelectHandler() {
			...
			public void onSelect(SelectEvent event) {
				...
				columnSelector
						.setSelectionMode(SelectionMode.MULTI_PERMIT_DOUBLES);
				columnSelector.setSelectedItems(store.getAll());
			}
		});
	...
	private ColumnConfig<ColumnDto, String> addDimensionColumn(List<ColumnConfig<ColumnDto,?>> columns) {
		ColumnConfig<ColumnDto,String> ccDimension = new ColumnConfig<ColumnDto,String>(ColumnDtoPA.INSTANCE.dimension(), 170, FilterMessages.INSTANCE.dimension()); 
	...
	private void configDimensionColumnEditing(ColumnConfig<ColumnDto,String> ccDimension, GridEditing<ColumnDto> editing) {
	...
	private CCContainer<ColumnDto, AggregateFunctionDto> addAggregateColumn(List<ColumnConfig<ColumnDto,?>> columns) {
		...
		CCContainer<ColumnDto, AggregateFunctionDto> cccAgg = gridHelperService.createComboBoxColumnConfig(values, ColumnDtoPA.INSTANCE.aggregateFunction(), true, SortDir.ASC, 120);
	...
	private void configAggregateColumnEditing(CCContainer<ColumnDto, AggregateFunctionDto> cccAgg, GridEditing<ColumnDto> editing) {
