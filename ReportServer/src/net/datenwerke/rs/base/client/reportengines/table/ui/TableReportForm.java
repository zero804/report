...
public class TableReportForm extends AbstractReportForm {
				...
				menu.addBeforeShowHandler(new BeforeShowHandler() {					
					...
					public void onBeforeShow(BeforeShowEvent event) {
						...
						addSpecialParameter(codeMirrorPanel, subMenuUser, subMenuItemUser, subMenuReport,
								...
								subMenuItemExpressions, datasourceFieldCreator);
					}
				});
			}
		});
	...
	private void addMetadataDatasourceField(Container fieldWrapper, boolean displayConfigFields) {
		metadataDatasourceFieldCreator = datasourceService.getSelectionField(fieldWrapper, displayConfigFields, datasourceTreeProvider);	
				...
				menu.addBeforeShowHandler(new BeforeShowHandler() {					
					...
					public void onBeforeShow(BeforeShowEvent event) {
						...
						addSpecialParameter(codeMirrorPanel, subMenuUser, subMenuItemUser, subMenuReport,
								...
								null, null, metadataDatasourceFieldCreator);
					}
				});
			}
		});
		...
		metadataDatasourceFieldCreator.addSpecificDatasourceConfig(new DatabaseSpecificFieldConfigExecution() {
			@Override
			public void executeGetColumns(final String query,
					final AsyncCallback<List<String>> callback) {
				
				TableReportDto report = (TableReportDto) getSelectedNode();
				tableReportDao.loadColumnDefinition(report, metadataDatasourceFieldCreator.getDatasourceContainer(), query, null, new RsAsyncCallback<List<ColumnDto>>(){
					@Override
					public void onSuccess(List<ColumnDto> result) {
						List<String> columns = new ArrayList<String>();
						for(ColumnDto col : result)
