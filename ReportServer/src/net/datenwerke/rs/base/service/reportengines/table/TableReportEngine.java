...
public class TableReportEngine extends ReportEngine<TableDataSource, TableOutputGenerator, TableMetadataExporter> {
	...
	private CompiledReport doExecuteWithSubtotals(OutputStream os, TableReport report, User user, ParameterSet parameters, String outputFormat, ReportExecutionConfig[] configs) throws ReportExecutorException {
		...
		for(Column col : report.getColumns())
			if(col.hasFilters() && null == col.getAggregateFunction() && ! col.isGroupedBy())
				throw new IllegalArgumentException(ReportEnginesMessages.INSTANCE.exceptionNoFilterOnNonAggregateNonGroupingForSubtotals());
		...
		RSTableModel subtotals = (RSTableModel) doExecute(os, subtotalReport, user, parameters, ReportExecutorService.OUTPUT_FORMAT_TABLE, configs);
		
		/* compute details */
		
		/* remove aggregate functions (and having filters and remove all ordering except for grouping columns */ 
		...
		return doExecuteNormal(os, detailReport, report, user, parameters, outputFormat, configs, subtotals, subtotalIndices, reducedSubtotalIndices, reducedGroupIndices);
	}
	...
	private CompiledReport doExecuteNormal(OutputStream os, TableReport report, User user,
			ParameterSet parameters, String outputFormat,
	 ...
	 * @param reducedSubtotalIndices The indices of subtotal columns for the reduced report (the one that was used to compute the subtotals, here non grouping, non aggregating columns were dropped)
	 * @return
	...
	private CompiledReport doExecuteNormal(OutputStream os, TableReport report, TableReport orgReport, User user,
			ParameterSet parameters, String outputFormat,
			ReportExecutionConfig[] configs, RSTableModel subtotals, int[] subtotalIndices, int[] reducedSubtotalIndices, int[] reducedGroupIndices) throws ReportExecutorException {
...
//		TableDataSource ds = dataSourceTransformer.transform(report, parameters);
		...
		try{
			...
			ds.addQueryComment("report: " + String.valueOf(null == orgReport.getId() ? orgReport.getOldTransientId() : orgReport.getId()));
			...
			if(null == subtotals)
				hasData = createReport(os, user, ds, (TableReport) report, outputGenerator, outputFormat, parameters, configs);
			else 
				hasData = createReportWithSubtotals(os, user, ds, (TableReport) report, orgReport, outputGenerator, outputFormat, parameters, configs, subtotals, subtotalIndices, reducedSubtotalIndices, reducedGroupIndices);
	...
	private void configureDataSource(TableDataSource ds, TableReport report, ParameterSet parameters, ReportExecutionConfig[] configs, User user) throws ReportExecutorException {
		...
		if(hasConfig(RECPaged.class, configs)){
			...
			ds.paged(paged.getPageSize()*(paged.getFirstPage()-1), (paged.getLastPage() - paged.getFirstPage() + 1)*paged.getPageSize());
		...
		if((null == ((TableReport)report).getColumns() || 
			((TableReport)report).getColumns().isEmpty() ) && ! ((TableReport)report).isSelectAllColumns())
	...
	private boolean createReport(OutputStream os, User user, TableDataSource ds, TableReport report, TableOutputGenerator outputGenerator, String outputFormat, ParameterSet parameters, ReportExecutionConfig... configs) throws ReportExecutorException {	
	...
	private boolean createReportWithSubtotals(OutputStream os, User user, TableDataSource ds, TableReport report, TableReport orgReport, TableOutputGenerator outputGenerator, String outputFormat, ParameterSet parameters, ReportExecutionConfig[] configs, RSTableModel subtotals, int[] subtotalIndices, int[] reducedSubtotalIndices, int[] reducedGroupIndices) throws ReportExecutorException {	
		...
		try{
			...
			firstDataRow: while(ds.next()){
				...
				for(int i = 0; i < rowSize; i++){
					...
					if(nrOfSubtotalGroupFields > groupFieldIndex && i == subtotalGroupFieldIndices[groupFieldIndex]){
						if(! isGroupRow(rowData[i], subtotals.getData().get(0).getRow(), reducedGroupIndices[groupFieldIndex]))
				...
				for(int i = 0; i < rowSize; i++){
					...
					if(nrOfSubtotalGroupFields > groupFieldIndex && i == subtotalGroupFieldIndices[groupFieldIndex]){
						if( (null != subtotalGroupFieldValues[groupFieldIndex] || null != rowData[i]) &&
							...
							if(! ((groupCount + 1) < subtotals.getData().size() && isGroupRow(rowData[i], subtotals.getData().get(groupCount + 1).getRow(), reducedGroupIndices[groupFieldIndex])))
			...
			if(foundData && subtotals.getData().size() != groupCount + 1)
				throw new ReportExecutorException("Database and ReportServer do not concur on the number of groups (" + (subtotals.getData().size()) + " != " + (groupCount + 1) + "). This may be a problem with your database settings. For example MySQL might compare strings case insensitive and ignore trailing spaces. ReportServer does not!");
	...
	private CellFormatter[] cleanFormatters(CellFormatter[] cellFormatters,
			TableOutputGenerator outputGenerator, int numberOfColumns) {
	...
	private void callPreHook(Report report, User user, TableDefinition td,
