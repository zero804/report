			...
			if(null == currencyList) {
				...
				for(Object o : currencyList){
					if((o instanceof String) && key.equals(o)){
						language = config.getString("localization.currencies.currency("+currencyList.indexOf(o)+")[@language]", defaultLanguages.get(key));
						region = config.getString("localization.currencies.currency("+currencyList.indexOf(o)+")[@region]", defaultRegions.get(key));
						break;
					}
				}
			}
