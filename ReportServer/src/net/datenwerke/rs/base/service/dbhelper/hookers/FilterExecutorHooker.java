...
public class FilterExecutorHooker implements DbFilterExecutorHook {
	...
	public QryCondition getQueryCondition(FilterSpec filter,
			Map<String, ParameterValue> pMap, QueryReplacementHelper prefixer, QueryBuilder queryBuilder, ManagedQuery managedQuery, Connection connection) {
		return getFilterCondition(((ColumnFilter)filter).getColumn(), pMap, prefixer, queryBuilder, managedQuery);
	}
	
	private QryCondition getFilterCondition(Column column,
		...
		if(null != column.getFilter()){
			...
			/* exclude values */
			Collection<QryCondition> excludeValueConditions = getExcludeValueCondition(pMap, prefixer, column, queryBuilder, managedQuery);
			...
			/* exclude ranges */
			Collection<QryCondition > excludeRangeConditions = getExcludeRangeConditions(pMap, prefixer, column, queryBuilder, managedQuery);
		...
		if(null != column.getNullHandling()){
			...
			if(column.getNullHandling().equals(NullHandling.Exlude)){
				innerCondition = managedQuery.addQryConditionAND(innerCondition, queryBuilder.getNewNotQueryCondition(inqc), queryBuilder);
			}else{
	...
	private Query nestedAggregatedInnerQuery(Column column, AggregateFunction aggregateFunction, Object data, Map<String, ParameterValue> pMap, QueryReplacementHelper prefixer, ManagedQuery managedQuery){
		QueryBuilder nested = managedQuery.getDbHelper().createNestedQuery(managedQuery.getInnerQuery(), pMap, prefixer, managedQuery.getPlainColumnNames());
		...
		/* select same column twice, as adding the filter to the 
		 * aggregated column would result in a HAVING filter */
	...
	private List<QryCondition> getExcludeValueCondition(Map<String, ParameterValue> pMap,
		...
		if(! nonWildcards.isEmpty()){
			excludeConditions.add(
					queryBuilder.getNewNotQueryCondition(
							queryBuilder.getNewInQueryCondition(column, nonWildcards)));
		...
		for(String wildcard : wildcards){
			excludeConditions.add(
					queryBuilder.getNewNotQueryCondition(
							queryBuilder.getNewLikeQueryCondition(column, wildcard)));
		}
	...
	private String resolveFilterReplacements(String filter, Column column, QueryBuilder queryBuilder, ManagedQuery managedQuery) {
		if(null == filter)
			return null;
	...
	private List<?> resolveFilterReplacements(List<String> filters, Column column, QueryBuilder queryBuilder, ManagedQuery managedQuery) {
		...
		for(String filter : filters){
			...
			if(object instanceof Collection)
				...
				if(object instanceof QryCondition)
					replaced.add(object);
				else
	...
	private QryCondition getIncludeCondition(Map<String, ParameterValue> pMap,
	...
	private void addIncludeRange(List<QryCondition> includeConditions, Column column, FilterRange filterRange, QueryBuilder queryBuilder, QueryReplacementHelper prefixer, Map<String, ParameterValue> pMap, ManagedQuery managedQuery) {
	...
	private Collection<QryCondition> getExcludeRangeConditions(
			Map<String, ParameterValue> pMap, QueryReplacementHelper prefixer,
			Column column, QueryBuilder queryBuilder, ManagedQuery managedQuery) {
		/* prepare list */
	...
	public String getValueWithConvertedWildcard(String value){
		if(null == value)
			return null;
		return value.replace(WILDCARD, SQL_WILDCARD)
				.replace(CHARACTER_WILDCARD, SQL_CHARACTER_WILDCARD);
	}
