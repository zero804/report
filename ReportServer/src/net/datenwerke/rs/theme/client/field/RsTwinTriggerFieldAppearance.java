...
public class RsTwinTriggerFieldAppearance extends Css3TwinTriggerFieldAppearance {
	...
	public void render(SafeHtmlBuilder sb, String value, FieldAppearanceOptions options) {
		...
		if (hideTrigger) {
			...
			sb.appendHtmlConstant("<div class='" + style.wrap() + "' style='padding-right:" + (triggerWidth + rightPadding) + "px;'>");
			renderInput(sb, value, inputStylesBuilder.toSafeStyles(), options);

			int right = fieldDetails.borderWidth() + 1;
			int height = -22;
			SafeStyles triggerWarpStyle = SafeStylesUtils.fromTrustedString("margin-top:" + height + "px;right:" + right + "px;" + "position:absolute; width: " + triggerWidth + "px;");
			sb.appendHtmlConstant("<div class='" + getStyle().triggerWrap() + "' style='" + triggerWarpStyle.asString() + "'>");


			sb.appendHtmlConstant("<div class='rs-field-ttf-t " + getStyle().trigger() + "' style='display: inline; position: relative; width: 16px;'>").append(triggerIcon.toSafeHtml()).appendHtmlConstant("</div>");
			if(! hideTwinTrigger)
				sb.appendHtmlConstant("<div class='rs-field-ttf-tt " + getStyle().twinTrigger() + "' style='display: inline; position: relative; width: 16px;'>").append(twinTriggerIcon.toSafeHtml()).appendHtmlConstant("</div>");
