		...
		return new RsAsyncCallback<CommandResultDto>(){
			...
			public void onFailure(Throwable caught) {
				...
				if(caught instanceof CommandNotFoundExceptionDto)
					addResultText(new SafeHtmlBuilder().appendEscaped(TerminalMessages.INSTANCE.commandNotFound(value)).toSafeHtml());
				else
		...
		autocompleteHelper.autoComplete(value, cursorPos, new AutocompleteCallback() {
			...
			public void autocomplete(final String result) {
				...
				if(haveToInsertQuote){
					...
					setInputFieldValue(prefix.substring(0, spacePos) + "\"" + prefix.substring(spacePos) + result + value.substring(cursorPos));
				} else {
			...
			public void manyResults(AutocompleteResultDto result) {
				...
				if(resultList.addAll(result.getObjectEntries().getList()) && (result.getCmdEntries().getList().size() > 0 || result.getEntries().getList().size() > 0) )
					resultList.add("");
