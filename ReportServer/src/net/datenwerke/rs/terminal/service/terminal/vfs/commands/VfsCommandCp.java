		...
		try{
			...
			while (sourceLocations.hasNext()) {
				...
				if(target.exists() && ! target.isFolder())
					if ( ! ( sourceObject instanceof ReportVariant && targetObject instanceof Report && ! (targetObject instanceof ReportVariant) ) )
				...
				if(targetObject instanceof SecurityService)
					if(! securityService.checkRights((SecurityTarget)targetObject, Read.class, Write.class))
						throw new ViolatedSecurityException();
				
				boolean deepCopy = parser.hasOption("r");
				
				/* perform copy */
				List<VFSLocation> copiedFileLocations = target.getFilesystemManager().copyFilesTo(sourceLocation, target, deepCopy);
