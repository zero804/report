...
public class ComputedColumnsView extends ReportExecutorMainPanelView {
	...
	protected void importColumn(final ComputedColumnDto toImport) {
		...
		for(final ComputedColumnDto col : store.getAll()){
			if( name.equals(col.getName()) ){
				...
				ConfirmMessageBox cmb = new DwConfirmMessageBox(ComputedColumnsMessages.INSTANCE.importColumnConflictTitle(), ComputedColumnsMessages.INSTANCE.importColumnConflictMsg(name));
	...
	private void createTableGrid() {
		ColumnConfig<ComputedColumnDto, String> name = new ColumnConfig<ComputedColumnDto, String>(compColPa.name(), 150, BaseMessages.INSTANCE.propertyName());
		name.setMenuDisabled(true);
		
		ColumnConfig<ComputedColumnDto, String> description = new ColumnConfig<ComputedColumnDto, String>(compColPa.description(), 300, BaseMessages.INSTANCE.propertyDescription());
		description.setMenuDisabled(true);
		
		ColumnConfig<ComputedColumnDto, String> expression = new ColumnConfig<ComputedColumnDto, String>(compColPa.expression(), 300, ComputedColumnsMessages.INSTANCE.expressionLabel());
	...
	private void createToolbar() {
		...
		/* add column */
		DwTextButton addBlockButton = toolbarService.createSmallButtonLeft(ComputedColumnsMessages.INSTANCE.addColumn(), BaseIcon.CALCULATOR);
		...
		/* add column */
		DwTextButton duplicateBlockButton = toolbarService.createSmallButtonLeft(ComputedColumnsMessages.INSTANCE.duplicateColumnLabel(), BaseIcon.CLONE);
		...
		deleteAllColumnsItem.addSelectionHandler(new SelectionHandler<Item>() {
			@Override
			public void onSelection(SelectionEvent<Item> event) {
				ConfirmMessageBox cmb = new DwConfirmMessageBox(ComputedColumnsMessages.INSTANCE.removeAllConfirmHeading(), ComputedColumnsMessages.INSTANCE.removeAllConfirmText());
				cmb.addDialogHideHandler(new DialogHideHandler() {
					@Override
					public void onDialogHide(DialogHideEvent event) {
						if (event.getHideButton() == PredefinedButton.YES) 
		...
		store.addStoreClearHandler(new StoreClearHandler<ComputedColumnDto>() {
			...
			public void onClear(StoreClearEvent<ComputedColumnDto> event) {
				...
				for(ColumnDto col : columns)
					if(col instanceof ColumnReferenceDto && (((ColumnReferenceDto)col).getReference()) instanceof ComputedColumnDto)
						toRemove.add(col);
	...
	protected void displayEditColumnDialog(final ComputedColumnDto selectedColumn, final boolean removeOnCancel) {
		...
		form.addField(String.class, ComputedColumnDtoPA.INSTANCE.description(), BaseMessages.INSTANCE.propertyDescription(), new SFFCTextAreaImpl(){
			@Override
		...
		form.addField(String.class, ComputedColumnDtoPA.INSTANCE.expression(), ComputedColumnsMessages.INSTANCE.expressionLabel(),
		new SFFCCodeMirror() {
			...
			public ToolBarEnhancer getEnhancer() {
				return new ToolBarEnhancer(){
					...
					public void enhance(ToolBar toolbar, final CodeMirrorPanel codeMirrorPanel) {
						DwTextButton selectColBtn = toolbarService.createSmallButtonLeft(ComputedColumnsMessages.INSTANCE.selectColBtnLabel(), BaseIcon.TABLE_ADD);
						...
						selectColBtn.addSelectHandler(new SelectHandler() {
							...
							public void onSelect(SelectEvent event) {
								...
								if(null == columnSelector){
									...
									columnSelector.setSelectionMode(net.datenwerke.gxtdto.client.forms.selection.SelectionMode.MULTI_PERMIT_DOUBLES);
									columnSelector.setSelectionCallback(new ItemsSelectedCallback<ColumnDto>() {
										@Override
										public void itemsSelected(
												List<ColumnDto> items) {
											if(null == items)
												return;
				...
				return new Validator<String>() {
					...
					public List<EditorError> validate(Editor<String> editor,
						...
						if("".equals(value)){
							...
							list.add(new DefaultEditorError(editor, ComputedColumnsMessages.INSTANCE.expressionNotEmpty(), value));
							return list;
						}
						return null;
					}
