...
public class SeriesConfigurationForm extends DwContentPanel implements Validatable, WizardResizer{
	...
	public enum SeriesSubPattern{
		DAILY_EVERY_Nth_DAY,
		DAILY_WORKDAY,
		MONTHLY_Nth_DAY_Mth_MONTH,
		MONTHLY_Nth_DAY_OF_WEEK_Mth_MONTH,
		NOT_SPECIFIED,
		YEARLY_AT_DATE,
		YEARLY_Nth_DAY_OF_WEEK_IN_MONTH
	}
...
//	private ToggleGroup weeklyDays;
	...
	public SeriesConfigurationForm(
		...
		allFields.addAll(Arrays.asList(new Object[]{dailyN, endType, firstExecution, lastExecution, monthlyM, monthlyN, monthlyNth,
						monthlyO, monthlyDay, numberOfExecutions, seriesType, weeklyN, weeklyDays,
						yearlyN, yearlyMonth, yearlyNth, yearlyDay, yearlyMonth2, atTime, timeRangeStart,
						timeRangeEnd, timeRangeInterval, timeRangeUnit, dailyRepeatType}));

		/* disallow blank as a valid value for all fields */
		for(Object f : allFields){
			if(f instanceof TextField)
				((TextField) f).setAllowBlank(false);
		}
	...
	/**
	 * attaches a listener to all RadioGroups which enables field according to the radio.config
	 */
	private void attachFieldConfigurator(){
		...
		ValueChangeHandler<HasValue<Boolean>> handler = new ValueChangeHandler<HasValue<Boolean>>() {
			public void onValueChange(ValueChangeEvent<HasValue<Boolean>> event){
				 ...
				 * change events, so it is possible to receive a change event from a radio 
				 * group currently in an invalid state. ignore these */
				
				if(null == getDailyRepeatType() ||
				   null == getSeriesType() ||
				   null == getSeriesSubType() ||
				   null == getEndType())
					return;
	...
	private Widget createTimeFieldSet(ReportScheduleDefinition definition) {
		...
		/* Zwischen 16:30 und 17:45 all 15 Minuten*/
		...
		/* set default value for radio group - must be done after adding it to togglegroup */
	...
	private Widget createDailyPanel(){
		if(null == dailyPanel){
			...
			Label txtWorkingDays = createLabel(SchedulerMessages.INSTANCE.every()+" "+SchedulerMessages.INSTANCE.workingday()); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
...
//		flexData2.setFlex(2);
	...
	public void setDailyRepeatType(DailyRepeatTypeDto type) {
		for(HasValue<Boolean> radio : dailyRepeatType){
			if(((Radio)radio).getData(ENUM_VALUE) == type){
				((Radio)radio).setValue(true, true);
				break;
			}
