...
public class LogSetupHelper {
	...
	public static void InitRsLogging(){
		...
		ClassLoader[] classloaders = new ClassLoader[]{LoggerFactory.class.getClassLoader().getSystemClassLoader(), cphelper.getClassloader()};
		...
		if(null != tomcatRsHandler){
			...
			try {
				...
				for(ClassLoader classloader : classloaders){
					try{
						Class<LoggerFactory> lfc = (Class<LoggerFactory>) classloader.loadClass(LoggerFactory.class.getCanonicalName());
						Object context = lfc.getMethod("getILoggerFactory").invoke(lfc);

						try{
							Object configurator = classloader.loadClass("ch.qos.logback.classic.joran.JoranConfigurator").newInstance();
							configurator.getClass().getMethod("setContext", classloader.loadClass("ch.qos.logback.core.Context")).invoke(configurator, context);
							...
							if(!classloader.loadClass("ch.qos.logback.core.joran.spi.JoranException").isAssignableFrom(e.getClass()))
								throw e;
						}

						Class<?> statusprinter = classloader.loadClass("ch.qos.logback.core.util.StatusPrinter");
						statusprinter.getMethod("printInCaseOfErrorsOrWarnings", classloader.loadClass("ch.qos.logback.core.Context")).invoke(statusprinter, context);
					}catch(Exception e){}
				}
