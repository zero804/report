...
public class OracleNamingStrategy extends DefaultComponentSafeNamingStrategy {
    ...
    public String collectionTableName(String ownerEntity,
    	...
    	String lookupName = new StringBuilder()
    		.append(ownerEntityTable)
    		.append('|') //$NON-NLS-1$
    		.append(propertyName)
    	...
    	String key = new StringBuilder()
    			.append(ownerEntity).append('|') //$NON-NLS-1$
    	...
    	if(null == tableName)
    		tableName = abbreviateName(super.collectionTableName(
    	...
    	if(assignedJoinTableNameMap.containsKey(tableName) && ! assignedJoinTableNameMap.get(tableName).equals(key))
    		throw new RuntimeException("Name clash when making up a name (" + tableName + ") for join table: " + key + ".\nName clashes with " + assignedJoinTableNameMap.containsKey(tableName)); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    ...
    public String logicalCollectionTableName(String tableName,
            ...
            String propertyName) {
        return null != tableName && tableName.startsWith(TABLE_PREFIX) ? "" : TABLE_PREFIX + abbreviateName(super.logicalCollectionTableName(
    ...
    public String classToTableName(String aClassName) {
    	String name = aClassName.startsWith(TABLE_PREFIX) ? "" : TABLE_PREFIX + abbreviateName(super.classToTableName(addUnderscores(aClassName)));
