...
public class DadgetCatalog extends DwWindow {
	...
	interface DashboardTemplates extends XTemplates{
		@XTemplate("<div class=\"rs-dadget-catalog-wrap\">" + 
		    "<div class=\"rs-dadget-icon\">{icon}</div>" +
		    "<div class=\"x-editable rs-dadget-title\">{title:nullsafe}</div>" +
		    "<div class=\"rs-dadget-description\">{description:nullsafe}</div>" +
		    "</div>")
	...
	@Inject
	public DadgetCatalog(
		HookHandlerService hookHandler,
		@Assisted DashboardDto dashboard,
	...
	private void initializeUI() {
		...
		/* load dadgets */
		ListStore<DadgetProcessorModel> dadgetStore = new ListStore<DadgetProcessorModel>(new BasicObjectModelKeyProvider<DadgetProcessorModel>());
		int dadgetCnt = 0;
		...
		final ListView<DadgetProcessorModel, DadgetProcessorModel> view = new ListView<DadgetProcessorModel, DadgetProcessorModel>(dadgetStore, new IdentityValueProvider<DadgetProcessorModel>()){
			@Override
		...
		view.setCell(new AbstractCell<DadgetProcessorModel>() {
			...
			public void render(com.google.gwt.cell.client.Cell.Context context,
					DadgetProcessorModel value, SafeHtmlBuilder sb) {
