...
public class EnhanceFilterToolbarHooker implements FilterViewEnhanceToolbarHook {
	...
	public void enhanceToolbarLeft(ToolBar toolbar) {
		...
		editTemplateBtn.addSelectHandler(new SelectHandler() {
			
			@Override
			public void onSelect(SelectEvent event) {
				displayEditTemplateDialog();
			}
		});
	...
	protected void displayEditTemplateDialog() {
		...
		/* create store */
		templateStore = new ListStore<TableReportTemplateDto>(new BasicObjectModelKeyProvider<TableReportTemplateDto>());
	...
	private ToolBar createToolbar() {
		...
		addTemplateBtn.addSelectHandler(new SelectHandler() {
			@Override
			public void onSelect(SelectEvent event) {
				displayAddTemplateDialog();
			}
		});
		...
		removeTemplatesBtn.addSelectHandler(new SelectHandler() {
			
			@Override
			public void onSelect(SelectEvent event) {
				TableReportTemplateDto template = grid.getSelectionModel().getSelectedItem();
				if(null != template){
					List<TableReportTemplateDto> toRemove = new ArrayList<TableReportTemplateDto>();
		...
		downloadTemplatesBtn.addSelectHandler(new SelectHandler() {
			@Override
			public void onSelect(SelectEvent event) {
				downloadTemplate();
			}
	...
	private Grid<TableReportTemplateDto> createGrid() {
		...
		/* create grid */
		Grid<TableReportTemplateDto> grid = new Grid<TableReportTemplateDto>(templateStore, new ColumnModel<TableReportTemplateDto>(columns));
		...
		grid.getSelectionModel().addSelectionChangedHandler(new SelectionChangedHandler<TableReportTemplateDto>() {
			...
			public void onSelectionChanged(
					SelectionChangedEvent<TableReportTemplateDto> event) {
	...
	protected void displayEditTemplate(TableReportTemplateDto template) {
		...
		DwTextButton submitBtn = new DwTextButton(BaseMessages.INSTANCE.submit(), new SelectHandler() {
			@Override
			public void onSelect(SelectEvent event) {
				form.mask(BaseMessages.INSTANCE.storingMsg());
				form.submit();				
	...
	protected FormPanel createEditForm(final SubmitCompleteCallback callback, TableReportTemplateDto template){
		...
		Collection<TableTemplateClientProviderHook> templateProvider = hookHandler.getHookers(TableTemplateClientProviderHook.class);
		
		ListStore<ModelClass> typeStore = new ListStore<ModelClass>(new BasicObjectModelKeyProvider<ModelClass>());
		for(TableTemplateClientProviderHook provider :templateProvider ){
		...
		final ComboBox<ModelClass> templateTypeCombo = new ComboBox<ModelClass>(new ComboBoxCell<ModelClass>(typeStore, new LabelProvider<ModelClass>() {
			@Override
			public String getLabel(ModelClass item) {
				return item.getName();
			}
		}, view));
		...
		if(null == template){
			...
			for(ModelClass model : typeStore.getAll()){
				if(model.getType().equals(template.getTemplateType())){
					templateTypeCombo.setValue(model);
					break;
				}
			}
		}
