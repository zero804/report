...
public class TsDiskMainComponent extends DwBorderContainer {
	...
	private void initStores() {
		...
		StoreSortInfo<AbstractTsDiskNodeDto> ssi = new StoreSortInfo<AbstractTsDiskNodeDto>(new Comparator<AbstractTsDiskNodeDto>() {
			@Override
	...
	private void initTree(){
		...
		treeSelectionModel.addSelectionChangedHandler(new SelectionChangedHandler<AbstractNodeDto>() {
			...
			public void onSelectionChanged(
					SelectionChangedEvent<AbstractNodeDto> se) {
...
//		explorerToolbar.add(new FillToolItem());
	...
	public void setMenuChanger(final Component comp, final ItemSelector itemSelector) {
		...
		comp.addBeforeShowContextMenuHandler(new BeforeShowContextMenuHandler() {
			@Override
			public void onBeforeShowContextMenu(BeforeShowContextMenuEvent event) {
				comp.setContextMenu(createContextMenu(itemSelector));
			}
		});
	}
	...
	public void moveNodes(List<AbstractTsDiskNodeDto> nodes, AbstractTsDiskNodeDto newParent) {
		...
		treeManagerDao.moveNodesAppend(new ArrayList<AbstractNodeDto>(nodes), newParent, new NotamCallback<List<AbstractNodeDto>>(TsFavoriteMessages.INSTANCE.movedNodes()));
	}
	...
	protected void initToolbars() {
		...
		if(teamSpaceService.isUser(currentSpace)){
			DwTextButton addFolderBtn = toolbarService.createSmallButtonLeft(TsFavoriteMessages.INSTANCE.addFolderText(), BaseIcon.FOLDER_ADD);
			...
			if(teamSpaceService.isManager(currentSpace)){
				DwTextButton importReportBtn = toolbarService.createSmallButtonLeft(TsFavoriteMessages.INSTANCE.importReportText(), BaseIcon.REPORT_ADD);
		...
		Iterator<TsFavoriteMainToolbarEnhancerHook> iterator = hookHandlerService.getHookers(TsFavoriteMainToolbarEnhancerHook.class).iterator();
		while(iterator.hasNext()){
		...
		/* init views */
		String userPropViewId = loginService.getCurrentUser().getUserPropertyValue(TsDiskUIModule.USER_PROPERTY_VIEW_VIEW_ID);
	...
	public void displayAddFolderDialog() {
		...
		form.addField(String.class, 
				TsDiskFolderDtoPA.INSTANCE.name(), 
		...
		form.addField(String.class, 
				TsDiskFolderDtoPA.INSTANCE.description(), 
				TsFavoriteMessages.INSTANCE.newFolderDescriptionLabel(),
	...
	public void itemSelected(AbstractTsDiskNodeDto item){
		History.newItem(TsDiskUIModule.TEAMSPACE_SELECT_ITEM_HISTORY_TOKEN + "/ts:" + getCurrentSpace().getId() + "&sel:" + item.getId() + (null != getCurrentFolder() ? "&id" + getCurrentFolder().getId() : ""), false);
	...
	public void folderOpened(TsDiskFolderDto folder, final Long selectChildId, final boolean execute) {
		History.newItem(TsDiskUIModule.TEAMSPACE_SELECT_ITEM_HISTORY_TOKEN + "/ts:" + getCurrentSpace().getId() + (null != folder ? "&id:" + folder.getId() : "") + (null != selectChildId ? "&sel:" + selectChildId : ""), false);
	...
	public void importReport(ReportDto report, boolean copyReport, boolean reference) {
		...
		favoriteDao.importReport(currentSpace, currentFolder, report, copyReport, reference, new RsAsyncCallback<TsDiskReportReferenceDto>(){
			@Override
