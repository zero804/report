...
public class TsDiskUIStartup {
	...
	@Inject
	public TsDiskUIStartup(
		final HookHandlerService hookHandler,
		final Provider<TsDiskTeamSpaceApp> teamSpaceAppProvider,
		...
		final Provider<MenuAddFolderHooker> addFolderHooker,
		final Provider<MenuRenameHooker> menuRenameHooker,
		final Provider<MenuDeleteHooker> menuDeleteHooker,
		final Provider<MenuExportHooker> exportBtnHooker,
		final Provider<MenuOpenHooker> openBtnHooker,
		...
		final TsDiskTreeLoaderDao favoriteDao,
		...
		hookHandler.attachHooker(TsFavoriteMenuHook.class, menuRenameHooker);
		...
		/* history */
		historyService.addHistoryCallback(TsDiskUIModule.TEAMSPACE_SELECT_ITEM_HISTORY_TOKEN, 
				new HistoryCallback() {
												...
												favoriteDao.loadNodeById(id, new RsAsyncCallback<AbstractNodeDto>(){
													public void onSuccess(AbstractNodeDto result) {
														if(result instanceof TsDiskRootDto)
															((TsDiskMainComponent)app.getAppComponent()).folderOpened(null, selIdFinal, true);
														else
															((TsDiskMainComponent)app.getAppComponent()).folderOpened((TsDiskFolderDto) result, selIdFinal, true);		
													};
												});
											}catch (NumberFormatException e) {
											}
		...
		historyService.addHistoryCallback(TsDiskUIModule.TEAMSPACE_OPEN_ITEM_HISTORY_TOKEN, referenceCallCallbackProvider);
