...
public class JasperReportParameterProposerToolbarConfiguratorHooker implements MainPanelViewToolbarConfiguratorHook {
	...
	@Inject
	public JasperReportParameterProposerToolbarConfiguratorHooker(
		ToolbarService toolbarUtils,
		JasperUtilsDao jasperUtilsDao,
	...
	public void mainPanelViewToolbarConfiguratorHook_addLeft(
			final MainPanelView view, ToolBar toolbar, AbstractNodeDto selectedNode) {
		...
		/* add parameter */
		DwTextButton createPreviewBtn = toolbarUtils.createSmallButtonLeft(JasperMessages.INSTANCE.parameterProposalBtn(), BaseIcon.LIGHTBULB);
				...
				jasperUtilsDao.proposeParametersFor(report, new RsAsyncCallback<List<JasperParameterProposalDto>>() {
					...
					public void onSuccess(List<JasperParameterProposalDto> result) {
						if(null != result && ! result.isEmpty())
							...
							new DwAlertMessageBox(JasperMessages.INSTANCE.noProposalsFoundTitle(), JasperMessages.INSTANCE.noProposalsFoundText()).show();
					}
				});
			}
		});
	...
	protected void displayResults(final ParameterView view, final JasperReportDto report, List<JasperParameterProposalDto> proposals) {
		/* create store */
		final ListStore<JasperParameterProposalDto> proposalStore = new ListStore<JasperParameterProposalDto>(jasperPa.dtoId());
		...
		/* Name column */
		ColumnConfig<JasperParameterProposalDto,String> nameConfig = new ColumnConfig<JasperParameterProposalDto,String>(jasperPa.name(), 200, JasperMessages.INSTANCE.name()); 
		columns.add(nameConfig);
		
		/* Key column */
		ColumnConfig<JasperParameterProposalDto,String> keyConfig = new ColumnConfig<JasperParameterProposalDto,String>(jasperPa.key(), 200, JasperMessages.INSTANCE.key()); 
		...
		ColumnConfig<JasperParameterProposalDto,ParameterDefinitionDto> proposalConfig = new ColumnConfig<JasperParameterProposalDto,ParameterDefinitionDto>(jasperPa.parameterProposal(), 200, JasperMessages.INSTANCE.proposal());
		...
		/* create grid */
		final DeletableRowsGrid<JasperParameterProposalDto> grid = new DeletableRowsGrid<JasperParameterProposalDto>(proposalStore, new ColumnModel<JasperParameterProposalDto>(columns));
	...
	protected void addParametersFor(final ParameterView view, JasperReportDto report, List<JasperParameterProposalDto> proposalDtos) {
