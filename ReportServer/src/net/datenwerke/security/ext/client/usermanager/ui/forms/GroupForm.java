...
public class GroupForm extends FormView {
	...
	protected void initializeForm(FormPanel form, VerticalLayoutContainer fieldWrapper) {
		...
		fieldWrapper.add(new FieldLabel(descriptionField,BaseMessages.INSTANCE.propertyDescription()), new VerticalLayoutData(1,-1));
		
		fieldWrapper.add(SeparatorTextLabel.createText(""), new VerticalLayoutData(1,10)); // as margin
		
		/* users */
		DwContentPanel ufs = DwContentPanel.newLightHeaderPanel(UsermanagerMessages.INSTANCE.assignedUsers(), getUserGrid(role));
		fieldWrapper.add(ufs, new VerticalLayoutData(1,300));
		
		fieldWrapper.add(SeparatorTextLabel.createText(""), new VerticalLayoutData(1,10)); // as margin
		
		/* groups */
		DwContentPanel rfs = DwContentPanel.newLightHeaderPanel(UsermanagerMessages.INSTANCE.assignedRoles(), getGroupGrid(role));
		fieldWrapper.add(rfs, new VerticalLayoutData(1,300));

		fieldWrapper.add(SeparatorTextLabel.createText(""), new VerticalLayoutData(1,10)); // as margin
		
		/* ous */
		DwContentPanel oufs = DwContentPanel.newLightHeaderPanel(UsermanagerMessages.INSTANCE.memberOus(), getOuGrid(role));
		fieldWrapper.add(oufs, new VerticalLayoutData(1,300));
	}
	
	@Override
	protected String getHeader() {
		return UsermanagerMessages.INSTANCE.editRole() + (((GroupDto)getSelectedNode()) == null ? "" : " (" + getSelectedNode().getId() + ")");
	}
	...
	private Widget getUserGrid(GroupDto role) {
		...
		/* create grid */
		ToolBarEnhancedDeletableRowsGridForTreeItemSelection wrapper = new ToolBarEnhancedDeletableRowsGridForTreeItemSelection(userStore, new ColumnModel<UserDto>(columns), basicTreeForUserSelection, UserDto.class);
	...
	private Widget getOuGrid(GroupDto group) {
		...
		/* create grid */
		ToolBarEnhancedDeletableRowsGridForTreeItemSelection wrapper = new ToolBarEnhancedDeletableRowsGridForTreeItemSelection(ouStore, new ColumnModel<GroupDto>(columns), basicTreeForOuSelection, OrganisationalUnitDto.class);
		Grid<AbstractNodeDto> grid = wrapper.getGrid();
	...
	private Widget getGroupGrid(GroupDto role) {
		...
		/* create grid */
		ToolBarEnhancedDeletableRowsGridForTreeItemSelection wrapper = new ToolBarEnhancedDeletableRowsGridForTreeItemSelection(groupStore, new ColumnModel<GroupDto>(columns), basicTreeForGroupSelection, GroupDto.class);
		Grid<AbstractNodeDto> grid = wrapper.getGrid();
