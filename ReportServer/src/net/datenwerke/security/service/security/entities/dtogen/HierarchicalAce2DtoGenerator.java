...
import net.datenwerke.security.service.security.entities.dtogen.HierarchicalAce2DtoGenerator;

/**
 * Poso2DtoGenerator for HierarchicalAce
 *
 * This file was automatically created by DtoAnnotationProcessor, version 0.1
 */
@GeneratedType("net.datenwerke.dtoservices.dtogenerator.DtoAnnotationProcessor")
public class HierarchicalAce2DtoGenerator implements Poso2DtoGenerator<HierarchicalAce,HierarchicalAceDto> {

	private final Provider<DtoService> dtoServiceProvider;

	@Inject
	public HierarchicalAce2DtoGenerator(
		Provider<DtoService> dtoServiceProvider	){
		this.dtoServiceProvider = dtoServiceProvider;
	}

	public HierarchicalAceDto instantiateDto(HierarchicalAce poso)  {
		HierarchicalAceDto dto = new HierarchicalAceDto();
		return dto;
	}

	public HierarchicalAceDto createDto(HierarchicalAce poso, DtoView here, DtoView referenced)  {
		/* create dto and set view */
		final HierarchicalAceDto dto = new HierarchicalAceDto();
		dto.setDtoView(here);
		...
		if(here.compareTo(DtoView.NORMAL) >= 0){
			...
			/*  set inheritancetype */
			Object tmpDtoInheritanceTypeDtogetInheritancetype = dtoServiceProvider.get().createDto(poso.getInheritancetype(), referenced, referenced);
			dto.setInheritancetype((InheritanceTypeDto)tmpDtoInheritanceTypeDtogetInheritancetype);
			/* ask for a dto with higher view if generated */
			((DtoMainService)dtoServiceProvider.get()).getCreationHelper().onDtoCreation(tmpDtoInheritanceTypeDtogetInheritancetype, poso.getInheritancetype(), new net.datenwerke.gxtdto.server.dtomanager.CallbackOnDtoCreation(){
				public void callback(Object refDto){
					if(null != refDto)
						dto.setInheritancetype((InheritanceTypeDto)refDto);
				}
			});
