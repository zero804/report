...
public abstract class FormView extends MainPanelView {
	...
	public Component getViewComponent(AbstractNodeDto selectedNode) {
		...
		if(! (getSelectedNode() instanceof SecuredAbstractNodeDtoDec) || ((SecuredAbstractNodeDtoDec)getSelectedNode()).hasAccessRight(WriteDto.class)){
			...
			final DwTextButton bSubmit = new DwTextButton(BaseMessages.INSTANCE.apply());
			bSubmit.setWidth(80);
			bbar.add(bSubmit);  
	...
	/**
	 * Allows to perform processing after the standard ajax call is finished but before the form submit
	 * or unmask is done. 
	 * @param clearance
	 */
	...
	protected void doSaveNode(AbstractNodeDto selectedNode) {
		treeManager.updateNode(selectedNode, new NotamCallback<AbstractNodeDto>(ManagerhelperMessages.INSTANCE.updated()){
			@Override
