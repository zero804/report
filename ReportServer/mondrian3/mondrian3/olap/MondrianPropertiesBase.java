 ...
 *
 * <li>Modify the default <code>mondrian3.properties</code> file checked into
...
public abstract class MondrianPropertiesBase extends TriggerableProperties {
    ...
    protected static final String mondrianDotProperties = "mondrian3.properties";
    ...
    /**
     * Loads this property set from: the file "$PWD/mondrian3.properties" (if it
     * exists); the "mondrian3.properties" in the CLASSPATH; and from the system
     * properties.
     */
    public void populate() {
        // Read properties file "mondrian3.properties", if it exists. If we have
        ...
        if (file.exists() && file.isFile()) {
            // Read properties file "mondrian3.properties" from PWD, if it
            // exists.
            try {
        ...
        if (url != null) {
            ...
            LOGGER.warn(
                "mondrian3.properties can't be found under '"
                + new File(".").getAbsolutePath() + "' or classloader");
        }

        // copy in all system properties which start with "mondrian3."
        int count = 0;
