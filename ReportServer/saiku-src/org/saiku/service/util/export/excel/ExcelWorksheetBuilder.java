...
public class ExcelWorksheetBuilder {
    ...
    private void addExcelTableRows(int startingRow) {
        ...
        setRowTotalAggregationCell(rowScanTotals, rowCount - 1, 0, true); // DW: ADAPTE
    }
            ...
            if (aggregatorsTable != null) {
            	if (grandTotal) {
                    ...
                    for (Integer colKey : colScanTotals.keySet()) {
						...
						for (int x1 = 0; x1 < colAggregator.length; x1++) {
							Measure measure = this.table.getSelectedMeasures()[x1];

							Cell cell = sheetRow.createCell(column + shift);
							String value = measure.getCaption() + " Grand Total";
							cell.setCellValue(value);
							cell.setCellStyle(totalsCS);
							shift++;
						}
    ...
    private int buildExcelTableHeader(int startRow) {
        ...
        for (x = 0; x < rowsetHeader.length; x++) {
            ...
            for (y = 0; y < maxColumns && y < rowsetHeader[x].length; y++) {
                ...
                int nextColumn = setColTotalAggregationCell(colScanTotals, sheetRow, x, column, false, x==rowsetHeader.length-1?true:false);
        ...
        if (topLeftCornerHeight > 0 && topLeftCornerWidth > 0) {
        	CellRangeAddress regionToMerge = new CellRangeAddress(startRow, startRow + topLeftCornerHeight - 1, 0, topLeftCornerWidth - 1);
        ...
        if (mergedItemsConfig.size() > 0) {
            for (ExcelMergedRegionItemConfig item : mergedItemsConfig) {
                ...
                CellRangeAddress regionToMerge = new CellRangeAddress(item.getStartY(),
