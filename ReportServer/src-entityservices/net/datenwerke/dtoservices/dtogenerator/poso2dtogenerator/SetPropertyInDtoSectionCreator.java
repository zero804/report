...
public class SetPropertyInDtoSectionCreator {
	...
	public void addSetSection(Collection<String> referenceAccu, MethodBuilder createDtoMethod) {
		...
		if(referencesPosoCollection()){
			...
			String collectionClassName = collection.getClass().getSimpleName() + "<" + referencedPoso.getDtoInformation().getClassName() + ">";
			
			String collectionVar = "col_" + getName();
			
			/* write method part */
			createDtoMethod.addBodyLine("final "  + collectionInterfaceName + " " + collectionVar + " = new " + collectionClassName + "();");
			createDtoMethod.beginBodyBlock("if( null != poso." + getGetMethod() + "())");
			
			createDtoMethod.beginBodyBlock("for(" + referencedPoso.getSimpleName() + " refPoso : poso." + getGetMethod() + "())");

			String tmpname = "tmpDto" + referencedPoso.getDtoInformation().getClassName() + getGetMethod();
			createDtoMethod.addBodyLine("final Object " + tmpname + " = dtoServiceProvider.get().createDto(refPoso, " + newHereDtoView + ", referenced);");
			createDtoMethod.addBodyLine(collectionVar + ".add((" + referencedPoso.getDtoInformation().getClassName() + ") " + tmpname + ");");
			createDtoMethod.addBodyComment("ask for dto with higher view if generated");
			
			createDtoMethod.addCustomLine("\t\t\t\t\t((DtoMainService)dtoServiceProvider.get()).getCreationHelper().onDtoCreation(" + tmpname + ", refPoso, new " + CallbackOnDtoCreation.class.getName() + "(){\n");
			...
			createDtoMethod.addBodyLine("throw new IllegalArgumentException(\"expected to get dto object (" + getName() + ")\");");
			...
			if(SourceFileGenerationUtils.isList(getReferencedPosoCollectionType())){
				...
				createDtoMethod.addBodyLine(collectionVar + ".set(tmp_index,(" + referencedPoso.getDtoInformation().getClassName() + ") dto);");
			} else {
				createDtoMethod.addBodyLine(collectionVar + ".remove(" + tmpname + ");");
				createDtoMethod.addBodyLine(collectionVar + ".add((" + referencedPoso.getDtoInformation().getClassName() + ") dto);");
			}
			...
			/* build collection class name */
			Collection collection = SourceFileGenerationUtils.instantiateCollection(SourceFileGenerationUtils.isCollection(collectionType));
			
			referenceAccu.add(collection.getClass().getName());
			
			String collectionClassName = collection.getClass().getSimpleName() + "<" + SourceFileGenerationUtils.getTypeArguments(collectionType) + ">";
			
			String collectionVar = "col_" + getName();
			
			/* write method part */
			createDtoMethod.addBodyLine(collectionInterfaceName + " " + collectionVar + " = new " + collectionClassName + "();");
			...
			createDtoMethod.addBodyLine("Object " + tmpname + " = dtoServiceProvider.get().createDto(poso." + getGetMethod() + "(), " + newHereDtoView + ", referenced);");
			createDtoMethod.addBodyLine("dto." + getSetMethodForDto() + "((" + referencedPoso.getDtoInformation().getClassName() + ")" + tmpname + ");");
			createDtoMethod.addBodyComment("ask for a dto with higher view if generated");
			createDtoMethod.addCustomLine("\t\t\t((DtoMainService)dtoServiceProvider.get()).getCreationHelper().onDtoCreation(" + tmpname + ", poso." + getGetMethod() + "(), new " + CallbackOnDtoCreation.class.getName() + "(){\n");
			createDtoMethod.incrementIndent();
			createDtoMethod.beginBodyBlock("public void callback(Object refDto)");
			
			createDtoMethod.beginOneLineBlock("if(null != refDto)");
			createDtoMethod.addBodyLine("dto." + getSetMethodForDto() + "((" + referencedPoso.getDtoInformation().getClassName() + ")refDto);");
				createDtoMethod.endBodyBlock();
				createDtoMethod.decrementIndent();
			createDtoMethod.addCustomLine("\t\t\t});\n");
			
		} else if(isString && disableHtmlEncode && enableSimpleHtmlPolicy){
			createDtoMethod.addBodyLine("dto." + getSetMethodForDto() + "(new HtmlPolicyBuilder().allowCommonInlineFormattingElements().allowCommonBlockElements().allowStyling().allowAttributes(\"size\", \"color\").onElements(\"font\").toFactory().sanitize(poso." + getGetMethod() + "() ));");
			...
			if(doNotCutOffStrings)
				createDtoMethod.addBodyLine("dto." + getSetMethodForDto() + "(StringEscapeUtils.escapeXml(poso." + getGetMethod() + "() ));");
			else
				createDtoMethod.addBodyLine("dto." + getSetMethodForDto() + "(StringEscapeUtils.escapeXml(StringUtils.left(poso." + getGetMethod() + "()," + DtoAnnotationProcessor.CUT_OFF_CLOBS_SIZE + ")));");
