...
public class Class2DtoSourceFileGenerator extends Poso2DtoSourceFileGenerator {
	...
	protected void addCreateDtoMethod(StringBuilder sourceBuilder) {
		...
		MethodBuilder createDtoMethod = new MethodBuilder(CREATE_DTO_METHOD_NAME, dtoClass, posoAnalizer.getSimpleName() + " poso", "DtoView here", "DtoView referenced" );
		...
		for(PosoAnalizer iFaces : posoAnalizer.getImplementedPosos()){
			for(ExposedClientMethod method : iFaces.getAllExposedInterfaceMethods()){
				if(method.isGetMethod()){
					...
					/* make sure that an annotation at a field wins over an annotation at an implementing class */
	...
	private void addSetSectionForRestrictionLevel(MethodBuilder createDtoMethod,
		...
		/* add section */
		createDtoMethod.beginBodyBlock("if(here.compareTo(" + view.getClass().getSimpleName() + "." + view.name() + ") >= 0)");
	...
	protected void addInstantiateDtoMethod(StringBuilder sourceBuilder) {
		...
		MethodBuilder method = new MethodBuilder(INSTANTIATE_DTO_METHOD_NAME, dtoClass, posoAnalizer.getSimpleName() + " poso");
