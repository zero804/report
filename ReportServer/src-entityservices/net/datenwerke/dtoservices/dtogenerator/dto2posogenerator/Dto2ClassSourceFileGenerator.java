...
public class Dto2ClassSourceFileGenerator extends Dto2PosoSourceFileGenerator {
	...
	protected void addAdditionalMemberVariables(StringBuilder sourceBuilder) {
		...
		for(PosoFieldDescriptor field : posoAnalizer.getAllExposedFields()){
			...
			int nr = 1;
			for(String customValidator : validator.getCustomValidators()){
				sourceBuilder.append("\tprivate final " + DtoPropertyValidator.class.getSimpleName() + " " + getValidatorName(nr, field) + ";\n");
				nr++;		
			}
	...
	private void addValidatorMethodFor(PosoFieldDescriptor field,
			StringBuilder sourceBuilder) {
		PropertyValidatorInformation validator = field.getPropertValidator();
		
		MethodBuilder method = new MethodBuilder(validator.getValidatorMethodName(), "boolean", getDtoType() + " dto", getPosoType() + " poso");
		...
		if(validator.hasTypeCheck()){
			...
			throwValidationFailedException(method, "type check failed for property " + field.getName(), "expected " + qName);
		}
		...
		if(! sValidator.isBypass()){
			...
			if(sValidator.getMinLength() != -1){
				...
				throwValidationFailedException(method, "String validation failed for " + field.getName(), "passed String is too short. Expected minimum length is: " + sValidator.getMinLength());
			}
			if(sValidator.getMaxLength() != -1){
				method.addBodyLine();
				method.beginOneLineBlock("if(((String)propertyValue).length() > " + sValidator.getMaxLength() + ") ");
				throwValidationFailedException(method, "String validation failed for " + field.getName(), "passed String is too long. Expected maximum length is: " + sValidator.getMaxLength());
			}
			if(! "".equals(sValidator.getRegex())){
				method.addBodyLine();
				method.beginOneLineBlock("if(! ((String)propertyValue).matches(\"" + sValidator.getRegex().replace("\\", "\\\\") + "\"))");
				throwValidationFailedException(method, "String validation failed for " + field.getName(), " Regex test failed.");
			}
		}
	...
	protected void addMergePoso(StringBuilder sourceBuilder) {
		MethodBuilder method = new MethodBuilder("mergePoso", "void", getDtoType() + " dto", "final " + getPosoType() + " poso");
		method.setThrows(ExpectedException.class.getSimpleName());
	...
	protected void addMergeUnmanagedPoso(StringBuilder sourceBuilder) {
		MethodBuilder method = new MethodBuilder("mergeUnmanagedPoso", "void", getDtoType() + " dto", "final " + getPosoType() + " poso");
		method.setThrows(ExpectedException.class.getSimpleName());
	...
	private void addMergePlainDto(StringBuilder sourceBuilder, boolean unmanaged) {
		...
		if(unmanaged)
			method = new MethodBuilder("mergePlainDto2UnmanagedPoso", "void", getDtoType() + " dto", "final " + getPosoType() + " poso");
		else
			method = new MethodBuilder("mergePlainDto2Poso", "void", getDtoType() + " dto", "final " + getPosoType() + " poso");
	...
	private void addMergeProxy(StringBuilder sourceBuilder, boolean unmanaged) {
		MethodBuilder method = null;
		if(unmanaged)
			method = new MethodBuilder("mergeProxy2UnmanagedPoso", "void", getDtoType() + " dto", "final " + getPosoType() + " poso");
		else
			method = new MethodBuilder("mergeProxy2Poso", "void", getDtoType() + " dto", "final " +  getPosoType() + " poso");
		method.setProtectedModifier();
		method.setThrows(ExpectedException.class.getSimpleName());
	...
	protected void addCreateUnmanagedPoso(StringBuilder sourceBuilder) {
		...
		if(null != posoAnalizer.getIdField() || posoAnalizer.hasIdFieldInHeritage()){
			...
			{
				method.addBodyLine("Field transientIdField = reflectionService.getFieldByAnnotation(poso, " + TransientID.class.getSimpleName() + ".class);");
		...
		if(null != posoAnalizer.getKeyField() || posoAnalizer.hasKeyFieldInHeritage()){
			...
			{
				method.addBodyLine("Field transientKeyField = reflectionService.getFieldByAnnotation(poso, " + TransientKey.class.getSimpleName() + ".class);");
