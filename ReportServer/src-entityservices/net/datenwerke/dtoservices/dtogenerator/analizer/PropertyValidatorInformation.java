...
public class PropertyValidatorInformation {
	...
	/**
	 * Wraps information on {@link net.datenwerke.dtoservices.dtogenerator.annotations.NumberValidator}
	 *
	 */
	...
	/**
	 * Wraps information on {@link net.datenwerke.dtoservices.dtogenerator.annotations.StringValidator}
	 *
	 */
		...
		if(fieldDescriptor.isExposedToClient()){
			...
			for(ExecutableElement key : mirror.getElementValues().keySet()){
				...
				if(key.toString().equals("validateDtoProperty()")){
					...
					for(ExecutableElement propValidatorKey : elementUtils.getElementValuesWithDefaults(propertyValidatorMirror).keySet()){
						if( "type()".equals(propValidatorKey.toString()))
							typeCheck = elementUtils.getElementValuesWithDefaults(propertyValidatorMirror).get(propValidatorKey);
						else if( "string()".equals(propValidatorKey.toString())){
							/* build string validator */
							AnnotationValue stringCheck = elementUtils.getElementValuesWithDefaults(propertyValidatorMirror).get(propValidatorKey);
							AnnotationMirror stringAnnoMirror = (AnnotationMirror) stringCheck.getValue();

							for(ExecutableElement stringValKey : elementUtils.getElementValuesWithDefaults(stringAnnoMirror).keySet()){
								AnnotationValue stringValValue = elementUtils.getElementValuesWithDefaults(stringAnnoMirror).get(stringValKey);
							...
							AnnotationValue numberCheck = elementUtils.getElementValuesWithDefaults(propertyValidatorMirror).get(propValidatorKey);
							AnnotationMirror numberAnnoMirror = (AnnotationMirror) numberCheck.getValue();

							for(ExecutableElement numberValKey : elementUtils.getElementValuesWithDefaults(numberAnnoMirror).keySet()){
								AnnotationValue numberValValue = elementUtils.getElementValuesWithDefaults(numberAnnoMirror).get(numberValKey);
							...
							AnnotationValue customValidatorsAnno = elementUtils.getElementValuesWithDefaults(propertyValidatorMirror).get(propValidatorKey);
	...
	public String getValidatorMethodName() {
		return "validate" + fieldDescriptor.getName().substring(0,1).toUpperCase() + fieldDescriptor.getName().substring(1) + "Property";
	}
	...
	public boolean hasTypeCheck() {
		return ! Object.class.getName().equals(SourceFileGenerationUtils.getQualifiedNameWithoutTypeArguments((DeclaredType) typeCheck.getValue()));
	}
