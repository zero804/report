...
@SupportedOptions({
	HookAdapterProcessor.BUILD_HOOK_ADAPTORS
})
public class HookAdapterProcessor extends AbstractProcessor {
	...
	private boolean decoratorExists(SourceFileGeneratorImpl generator) {
		try {
			FileObject file = processingEnv.getFiler().getResource(StandardLocation.SOURCE_OUTPUT,  generator.getPackageName(), generator.getClassName() + ".java");
